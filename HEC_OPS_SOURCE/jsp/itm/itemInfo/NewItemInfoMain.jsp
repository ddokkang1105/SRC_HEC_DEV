...
<script type="text/javascript">
	...
	function goItemInfoEdit() {
		if (isPossibleEdit == "Y") { 
			...
			var target = "actFrame";
			var data = "s_itemID=${s_itemID}&languageID=${sessionScope.loginInfo.sessionCurrLangType}&showVersion=${showVersion}";
	...
	*/
	...
	function goMenu(avg) {
		...
		if (avg == "fileMgt") {
			url = "goFileMgt.do?&fileOption=${menuDisplayMap.FileOption}&itemBlocked=${itemBlocked}"; // 관련문서
	   ...
	   <li class="floatR pdR20"> 
	   		...
	   		<c:otherwise>
	   			<c:if test="${itemBlocked eq '0' && sessionScope.loginInfo.sessionLogintype == 'editor' and myItem == 'Y'}">
		   			&nbsp;
		   			<span class="btn_pack nobg"><a href="javascript:goItemInfoEdit();"class="edit mgT2" title="Attributes"></a></span>		   		
	     	    </c:if>
			   </c:otherwise>
			</c:choose>	   		
	   	  	<c:if test="${menuDisplayMap.FileOption ne  'N'}">
		  		<span class="icon" alt="FILEMGT" title="Files"  style="cursor:pointer;_cursor:hand"><img onclick="goMenu('fileMgt');" class="topIconImg" alt="imgFILEMGT" src="${root}${HTML_IMG_DIR}/sc_file.png"></img></span>
		  		<font color="#1141a1">(${menuDisplayMap.FILE_CNT})</font>
		  	</c:if>
		   	<c:if test="${menuDisplayMap.HasDimension eq '1' && itemBlocked eq '0' && myItem == 'Y'}">
		  		<span class="icon" alt="DIMENSION" title="Dimension"  style="cursor:pointer;_cursor:hand"><img onclick="goMenu('dim');" class="topIconImg" alt="imgDIMENSION" src="${root}${HTML_IMG_DIR_ITEM}/img_cube.png"></img></span>
		  		<font color="#1141a1">(${menuDisplayMap.DIM_CNT})</font>
		  	</c:if>
		  	<c:if test="${isExistModel eq 'Y'}">
		  		<span class="icon" alt="Occurrence" title="Occurrence"  style="cursor:pointer;_cursor:hand"><img onclick="goMenu('model');" src="${root}${HTML_IMG_DIR}/img_folderClosed.png"></img></span>
		  		<font color="#1141a1">(${MDL_CNT})</font>
		  	</c:if>
		  	<c:if test="${changeMgt ne '1' && curChangeSet ne '' && itemInfo.Status ne 'NEW1' && itemInfo.Status ne 'NEW2'}">
	   			<span class="icon" alt="Revision" title="Revision"  style="cursor:pointer;_cursor:hand"><img onclick="goMenu('rev');" src="${root}${HTML_IMG_DIR}/img_revision.png"></img></span>
		  		<font color="#1141a1">(${REV_CNT})</font>
		  	</c:if>
	   </li>
    </div>
	...
	<table style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0" border="0" class="tbl_preview">
		<c:forEach var="prcList" items="${prcList}" varStatus="status">
			...
			<tr>				
				<!-- 관리조직 -->
				<th>${menu.LN00018}</th>
				<td style="cursor:pointer;"  OnClick="fnOpenTeamInfoMain(${prcList.OwnerTeamID})" >${prcList.OwnerTeamName}</td>
				<input type="hidden" id="orderTeamName" name="orderTeamName"  value="${prcList.OwnerTeamName}"/>
				<!-- 담당자 -->	
				<th>${menu.LN00004}</th>			
				<c:if test="${GlobalVal.EMP_PHOTO_ITEM_DISPLAY ne 'Y' || roleAssignMemberList.size() == 1}" >
				<td id="authorInfo" style="cursor:pointer;_cursor:hand">${prcList.Name} </td>
				</c:if>
				<c:if test="${GlobalVal.EMP_PHOTO_ITEM_DISPLAY eq 'Y' && roleAssignMemberList.size() > 1}" > 
					<td>
			...
			<tr>
				...
				<td style="height:53px;" class="tdLast alignL pdL5" colspan="7">
					...
					<c:forEach var="relatedfileList" items="${pertinentFileList}" varStatus="status">
						<input type="checkbox" name="checkedFile" value="${relatedfileList.FileName}/${relatedfileList.FileRealName}//${relatedfileList.Seq}" class="mgL2 mgR2">
