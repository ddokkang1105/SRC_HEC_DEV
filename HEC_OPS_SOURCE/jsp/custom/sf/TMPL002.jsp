...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
	...
	function getTreeSubItems(subItems){
	 ...
	 function clickMainMenu(menuType, menuName, menuIcon, defDimValueID, menuStyle, scndMenuStyle, menuUrl, boardMgtID, mainType, tmplFilter, projectID, tmplType){ 
		...
		}else if(menuType == "MYPAGE"){url = "mySpaceV34.do?noticType=4&srID=${srID}&mainType=" +mainType+"&myPageTmplFilter=${myPageTmplFilter}"; name = "MYPAGE";
...
<div id="maincontainer">
	...
	<div id="topsection">
		...
		<div class="floatR" id="MemInfo">
			<ul>
				...
				<li class="pdR10 pdL20"><img src="${root}${HTML_IMG_DIR}/icon_admin.png" class="pdT10"/></li>
				<c:if test="${loginIdx == 'BASE'}" > 
				<li class="pdR10 pdT7" OnClick="fnChangePwd();" alt="USER" titl="USER"> <font style="color:#3d444e;font-weight:bold;"><u>${sessionScope.loginInfo.sessionUserNm}</u></font>&nbsp;(${sessionScope.loginInfo.sessionTeamName})</li> 
				</c:if>
				<c:if test="${loginIdx != 'BASE'}" > 
				<li class="pdR10 pdT7" alt="USER" titl="USER"> <font style="color:#0D65B7;font-weight:bold;"><u>${sessionScope.loginInfo.sessionUserNm}</u></font>&nbsp;(${sessionScope.loginInfo.sessionTeamName})</li> 
				</c:if>
				<li class="logout logoutbtn pdT7 mgR20"><a href="#" class="logout" ><img src="${root}${HTML_IMG_DIR}/menu_icon_logout.png" style="cursor:pointer"></a></li>       
				
				<c:if test="${sessionScope.loginInfo.sessionMlvl != 'VIEWER'}">
				<li class="pdR10 pdL20 pdT7"><img src="${root}${HTML_IMG_DIR}/icon_mypage.png"/></li>
				<li class="myPage pdT7 hypen" style="cursor:pointer;" alt="MYPAGE" title="MYPAGE">My Page</li>
				<li class="hypen pdT5">
				</c:if>
				<c:if test="${sessionScope.loginInfo.sessionMlvl == 'VIEWER'}">
				<li>
                    ...
                    <ul id="nav">
                        <li class="top"><span id="cmbLanguage">${sessionScope.loginInfo.sessionCurrLangNm}</span>
                            ...
                            <c:forEach var="lang" items="${langList}" varStatus="status" >
                                ...
                                <a href="#" alt="${lang.NAME}" style="width:75px;" id="${lang.CODE}">${lang.NAME}</a>
                                </li>
                            </c:forEach>                            
                            </ul>
                        </li>                       
						...
						<li class="top" style="width:60px;"><span>Menu</span>
							 <ul class="sub">
								...
								<li alt="${topMenu.MenuID}" title="${topMenu.Name}" onclick="clickMainMenu('${topMenu.MenuID}','${topMenu.Name}','','','','','${topMenu.URL}.do?${topMenu.FILTER}')">
									<a href="#" style="width:75px;" alt="${topMenu.Name}">${topMenu.Name}</a>
								</li>
								</c:forEach>
								<li class="help" alt="HELP" title="HELP" style="cursor:pointer"><a href="#" style="width:75px;">Help</a></li>
							 </ul>
						</li>
					</ul>
				</li-->
	...
	<!--END::TOP -->	
		...
		<div style="height:45px;" id="tmpl2_submenusection">
			<c:choose>
		       <c:when test="${templCode == ''}">
		     		<div id="slidemenu01" class="slidemenu" style="display:block"></div>
		    	</c:when>
			...
			<div class="menu_right" id="menuRight" style="display:block;position: fixed;top: 50px;right: 0;">
