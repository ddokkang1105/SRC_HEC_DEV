...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		fnSelect('subCategory', data+"&level=2", 'getESMSRCategory', '${srInfoMap.SubCategory}', 'Select', 'esm_SQL');
	...
	function fnGoSRList(){
		...
		var data = "srType=${srType}&scrnType=${scrnType}&srMode=${srMode}&pageNum=${pageNum}&projectID=${projectID}"
					...
					+ "&srReceiptTeam=${srReceiptTeam}&crReceiptTeam=${crReceiptTeam}&searchStatus=${searchStatus}&srStatus=${srStatus}&varFilter=${varFilter}&itemID=${itemID}&multiComp=${multiComp}&itemTypeCode=${itemTypeCode}";
	...
	function fnAccept(){
		var srID = "${srInfoMap.SRID}";
		var url = "acceptICP.do";
		var data = "srID="+srID+"&blockSR=Y";
		var target = "saveFrame";
		ajaxPage(url, data, target);
	}
</script>
...
<div id="processSRDIV"> 
	...
	<c:choose>
		...
		<c:otherwise>
			<div class="child_search01 pdL10">
				<img src="${root}${HTML_IMG_DIR}/icon_folder_upload_title.png">&nbsp;&nbsp;<b>${menu.LN00282}</b>
			</div>
		</c:otherwise>
	</c:choose>
	...
	<table class="tbl_brd" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0">
		...
		<tr>
			<!-- 변경 항목 -->
			<th class="alignL pdL10">${menu.LN00082}</th>
			<td class="sline tit last" colspan=3  id="icpItem" style="cursor:pointer;"><span style="color:blue;">${srInfoMap.SRArea3Code}&nbsp;${srInfoMap.SRArea3Name}</span>&nbsp;(&nbsp;${srInfoMap.srArea3Path}&nbsp;)</td>
			<!-- Item Type Code -->
			<th class="alignL pdL10">${menu.LN000LN00087}</th>
		...
		<tr>
			...
			<td class="sline tit last"><select id="subCategory" name="subCategory" class="sel"></select></td>
			<!-- 완료요청일 -->
		...
		<tr>
			<td colspan="6" class="tit last"><div style="height:200px; overflow: auto;" >${srInfoMap.Description}</div></td>
		</tr>
	</table>
	...
	<div class="alignBTN">				
		<c:if test="${(srInfoMap.Status == 'SPE018' || srInfoMap.Status == 'SPE019')}" >
			...
			<span class="btn_pack small icon"><span class="move"></span><input value="Transfer" type="submit" id="transferItem"></span>
		</c:if>
		<c:if test="${srInfoMap.Status eq 'SPE019' && srInfoMap.SRNextStatus eq 'SPE021' && appBtn eq 'Y'}" >
			<span class="btn_pack medium icon"><span class="confirm"></span><input value="Accept" type="submit" onclick="fnAccept()"></span>
			<span class="btn_pack medium icon"><span class="cancel"></span><input value="Reject" type="submit" onclick="fnReject()"></span>
		</c:if>
		<c:if test="${srInfoMap.SRNextStatus eq 'SPE020' && appBtn eq 'Y'}" >	<!-- 접수 다음 결재상신할 때-->
		...
		<c:if test="${srInfoMap.SRNextStatus eq srInfoMap.LastSpeCode && srInfoMap.SvcCompl eq 'N' && appBtn eq 'Y'}" >
	...
	* Process Log
	<table class="tbl_brd mgT5 mgB20" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0">
		<colgroup>
			<col width="10%">
		    <col width="20%">
		 	<col width="10%">
