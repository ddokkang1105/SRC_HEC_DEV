...
// 		}
	...
	function fnGoIcpList(){
		...
		var data = "srType=${srType}&scrnType=${scrnType}&srMode=${srMode}&pageNum=${pageNum}&projectID=${projectID}"
						...
						+"&srStatus=${srStatus}&status=${status}&searchStatus=${searchStatus}";
	...
	<div id="wrapView">
		<table class="tbl_brd" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0">
			...
			<tr>				
				...
				<td class="sline tit last" colspan="3" id="icpSubject">${srInfoMap.Subject}</td>
				<!-- SR No.  -->
				<th class="alignL pdL10">SR No.</th>
				<td class="sline tit last">${srInfoMap.SRCode}</td>
			</tr>
			<tr>
				<td colspan="6" class="tit last" Valign="Top"><div style="height:200px; overflow: auto;" >${srInfoMap.Description}</div></td>
			</tr>		
		</table>
		...
		<c:if test="${isPopup ne 'Y'}" >
			...
			<c:if test="${scrnType ne 'cust' }" >
				<span class="btn_pack medium icon"><span class="list"></span><input value="List" type="submit"  onclick="fnGoIcpList();"></span>
			</c:if>
		</c:if>	
		</div>
	</div>
