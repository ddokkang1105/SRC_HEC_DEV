...
<div id="viewSRDIV" <c:if test="${isPopup eq 'Y'}" >class="mgL10 mgR10 mgB10"</c:if>> 
	...
	<table class="tbl_brd" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0">
		...
		<tr>				
			...
			<td class="sline tit last" colspan="5" id="subject">${srInfoMap.Subject}</td>
		</tr>
	</table>
	...
	<c:if test="${isPopup ne 'Y'}" >
		...
		<c:if test="${srInfoMap.Status eq 'SPE004' && elmCodeMap.SPE005 ne 'Y' && srInfoMap.ReceiptUserID eq sessionScope.loginInfo.sessionUserId}" >				
		...
		<c:if test="${srInfoMap.Blocked eq 1 && spePrePostMap.SPE012 eq 'POST' && srInfoMap.ReceiptUserID == sessionScope.loginInfo.sessionUserId && ((elmCodeMap.SPE005 eq 'Y' && spePrePostMap.SPE005 eq 'PRE' && scrCLSYN eq 'Y' && spePrePostMap.SPE004 ne 'POST') || srInfoMap.SRNextStatus eq 'SPE012') }" >
		...
		<c:if test="${spePrePostMap.SPE003 eq 'PRE' && spePrePostMap.SPE012 eq 'POST' && elmCodeMap.SPE005 eq 'Y' && srInfoMap.ReceiptUserID eq sessionScope.loginInfo.sessionUserId }" >
		...
		<c:if test="${(srInfoMap.SRNextStatus eq 'SPE015' || srInfoMap.Status == 'SPE015') && elmCodeMap.SUC002 eq 'Y'  && srInfoMap.ReceiptUserID eq sessionScope.loginInfo.sessionUserId && createCSR eq 'N'}" >
			<span class="btn_pack medium icon"><span class="confirm"></span><input value="Commit" type="submit"  onclick="fnCommitItem();" style="cursor:hand;"></span>	
		</c:if>
		...
		<c:if test="${srInfoMap.SRNextStatus eq srInfoMap.LastSpeCode && srInfoMap.Status ne 'SPE002' && srInfoMap.SvcCompl eq 'N' && srInfoMap.ReceiptUserID == sessionScope.loginInfo.sessionUserId && createCSR eq 'N'}" >
