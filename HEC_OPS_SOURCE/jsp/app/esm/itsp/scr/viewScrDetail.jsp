...
<script type="text/javascript">
	...
	function fnClickedTab(avg) { 
		...
		var realMenuIndex = "1 2 3 4 5 6".split(' ');
...
// 			} 
	...
	function fnCompletionSCR(){		
		...
		var data = "scrID=${scrID}&screenMode=${screenMode}&scrStatus=CLS&srID=${srID}&speCode=SPE012";		
		var target = "scrDiv";
		ajaxPage(url, data, target);
	}
		...
		<table class="tbl_blue01 mgT10" style="table-layout:fixed;" width="98%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
			    ...
			    <td class="alignL pdL10" colspan="5">
			    	...
			    	현업 테스트 &nbsp;&nbsp;<input type="checkbox" id="finTestYN" name="finTestYN" value="${scrInfo.FinTestYN}" <c:if test="${scrInfo.FinTestYN == 'Y'}">checked</c:if> <c:if test="${screenMode eq 'V'}">disabled=true</c:if> /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			    	운영이관 &nbsp;&nbsp;<input type="checkbox" id="CTRYN" name="CTRYN" value="${scrInfo.CTRYN}" <c:if test="${scrInfo.CTRYN == 'Y'}">checked</c:if> <c:if test="${screenMode eq 'V'}">disabled=true</c:if> /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			    </td>	
