...
<script type="text/javascript">
	...
	function fnGoSRList(){
		...
		var data = "srType=${srType}&scrnType=${scrnType}&srMode=${srMode}&pageNum=${pageNum}&projectID=${projectID}"
					...
					+ "&srReceiptTeam=${srReceiptTeam}&crReceiptTeam=${crReceiptTeam}&searchStatus=${searchStatus}&srStatus=${srStatus}&fromSRID=${fromSRID}";
		var target = "mainLayer"; 
		if("${fromSRID}" != null &&"${fromSRID}" != ''){
			target = "esrListDIv";
		}
	...
	function fnCreateCSR(avg) {
		...
		var url = "registerCSR.do?&mainMenu=1&s_itemID=${s_itemID}&scrnType=${scrnType}&srID=${srInfoMap.SRID}&fromSR=Y&speCode=SPE015&blockSR=Y&srType=${srType}";
		var w = 1100;
		var h = 445;
	...
	function addSharer(){
		...
		var url = "selectMemberPop.do?docCategory=SR&projectId=${projectID}&s_memberIDs="+sharers;
...
<div id="processSRDIV"> 
	...
	<div class="alignBTN">				
		...
		<c:if test="${srInfoMap.SRNextStatus eq 'SPE015' && elmCodeMap.SUC003 eq 'Y' && sessionScope.loginInfo.sessionAuthLev < 3 }" >
		...
		<c:if test="${srInfoMap.SRNextStatus eq 'SPE008' && elmCodeMap.SUC002 eq 'Y' && createCSR eq 'N'}" >
