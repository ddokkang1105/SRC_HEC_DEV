...
<script type="text/javascript">
	...
	<c:forEach items="${csList}" var="list" varStatus="status">
		<ul onClick="fnGoItemInfo('${list.ItemID}')">
			<li style="width:13%;max-width:85px;">${list.TypeCode}</li>
			<li style="width:40%;">${list.Identifier}&nbsp;
			<c:choose>				
				<c:when test="${fn:length(list.ItemName) > 28}">
					${fn:substring(list.ItemName,0,28)}...
				</c:when>
				<c:otherwise>
					${list.ItemName}
				</c:otherwise>
			...
			<li style="width:15%;">${list.TeamName}</li>
