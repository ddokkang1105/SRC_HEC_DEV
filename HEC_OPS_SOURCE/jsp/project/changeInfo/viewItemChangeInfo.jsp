...
<script type="text/javascript">
	...
	function goAprvDetail() {
		var url = "${wfURL}.do";
		var data = "wfInstanceID=${getData.WFInstanceID}&actionType=view&changeSetID=${getData.ChangeSetID}&isMulti=N&wfDocType=CS&projectID=${ProjectID}&isProc="+$("#isProc").val();
		var target = "changeInfoViewFrm";
		ajaxPage(url, data, target);
	}		
	...
	function goApprovalPop() {
		var url = "${wfURL}.do?";
		var data = "isPop=Y&changeSetID=${getData.ChangeSetID}&isMulti=N&wfInstanceID=${getData.WFInstanceID}&wfDocType=CS&ProjectID=${getData.ProjectID}";
...
<div id="changeInfoViewDiv" class="hidden" style="overflow:auto; overflow-x:hidden; padding: 6px 6px 6px 6px;" >
	...
	<input type="hidden" id="isProc" name="isProc" value="" />
	...
	<div id="tblChangeSet" style="width:99%">
		...
		<table class="tbl_blue01 mgT10" style="table-layout:fixed;" width="100%" border="0" cellpadding="0" cellspacing="0">
			<table style="table-layout:fixed;" class="tbl_blue01 mgT10">
				...
				<tr>
					...
					<th  class="alignL pdL10">${menu.LN00290}</th>
						<td colspan="5" class="last alignL pdL5" style="width:100%;height:150px;">
							<textarea id="description" name="description" style="width:100%;height:150px;" readOnly>${getData.Description}</textarea>
						</td>
				</tr>
				 <tr>
		           <td colspan="6" class="alignR pdR20 last" bgcolor="#f9f9f9" >
		           &nbsp;<span class="btn_pack medium icon"><span class="confirm"></span><input value="현재 문서 보기" onclick="goItemPopUpNoID()" type="submit"></span><!--Veiw current Item-->
		         	  	<c:if test="${lastChangeSet == 'N' }">
		           		&nbsp;<span class="btn_pack medium icon"><span class="confirm"></span><input value="과거 문서 보기" onclick="fnOpenViewVersionItemInfo(${getData.ChangeSetID})" type="submit"></span><!--Veiw version Item-->
			            </c:if>
			            <c:if test="${getData.WFInstanceID != '' && getData.wfStatus != '0' }">
		           		&nbsp;<span class="btn_pack medium icon"><span class="confirm"></span><input value="결재문서" onclick="goAprvDetail()" type="submit"></span><!--View Approval Document-->
			            </c:if>
				        ...
				        <c:if test="${getData.AuthorID == sessionScope.loginInfo.sessionUserId}" > 
			               ...
			               <c:if test="${StatusCode == 'CMP' && ( getData.CheckInOption == '03' || getData.CheckInOption == '03A')}" >   
			...
			<c:if test="${nOdList.size() > 0 && lastChangeSet == 'Y' && getData.ChangeType == 'MOD' }">	
