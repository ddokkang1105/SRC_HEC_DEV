...
function gridOTInit(){		
	...
	OT_gridArea.enablePaging(true,40,10,"pagingArea",true,"recInfoArea");
...
function setOTGridData(){
	...
	result.title = "${title}";
	result.key = "config_SQL.getReportTypeCode";
	result.header = "${menu.LN00024},#master_checkbox,Code,${menu.LN00028},${menu.LN00035},Function,Output Type,Authority,Is Public,CNT";
	result.cols = "CHK|ReportCode|Name|Description|ReportURL|OutputType|Authority|IsPublic|CNT";
	result.widths = "50,50,80,200,*,130,80,80,80,0";
	result.sorting = "int,int,str,str,str,str,str,str,str,str";
	result.aligns = "center,center,center,left,left,left,center,center,center,center";
	result.data = "languageID=${sessionScope.loginInfo.sessionCurrLangType}" + "&pageNum=" + $("#currPage").val();	
	return result;
...
function gridOnRowOTSelect(id, ind){
	if(ind != 1){
		...
		var data = "ReportCode="+ OT_gridArea.cells(id, 2).getValue() + "&pageNum=" + $("#currPage").val();		 
...
function delRptType() {
	...
	if (confirm("${CM00004}")) {
		...
		if (itemTypeCodes != "") {
			...
			var data = "reportCodes=" + itemTypeCodes;
	...
	<div class="child_search01 mgL10 mgR10">
		<ul>
			...
			<c:if test="${sessionScope.loginInfo.sessionLogintype == 'editor'}">
					...
					<!-- 생성 아이콘 -->
					&nbsp;<span class="btn_pack small icon"><span class="add"></span><input value="Add" type="submit" onclick="AddReportTypePop();" ></span>
					<!-- 삭제 아이콘 -->
					&nbsp;<span class="btn_pack small icon"><span class="del"></span><input value="Del" type="submit" onclick="delRptType();" ></span>
			</c:if>	
			</li>
		</ul>
	</div>
