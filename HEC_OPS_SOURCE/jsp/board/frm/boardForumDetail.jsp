...
// 	}
	...
	$('#back').click(function(e){
		...
		var data = "languageID=${sessionScope.loginInfo.sessionCurrLangType}"
					...
					+"&screenType=${screenType}&projectID=${projectID}&listType=${listType}&srID=${srID}";
	...
	$('.layer_trigger').click(function(){
		...
		LeyerPopupView('dvd70_file', 'layerPopup', pos.top, -pos.left);
...
function editComment(boardID, ItemID) {
	if(confirm("${CM00005}")){
		var url = "boardNewComment.do";
		var data = "languageID=${sessionScope.loginInfo.sessionCurrLangType}" 
			+ "&s_itemID="+$('#s_itemID').val()
...
<form name="formDetailFrm" id="formDetailFrm" method="post" onsubmit="return false;">
	...
	<!-- comment 상세화면 -->
	  <table cellpadding="0" cellspacing="0" width="100%" class="tbl_comview">
		...
		<tr>
			<th>${boardMap.Subject}</th>
			<th class="ac">${boardMap.LastUpdated}</th>
			<c:if test="${sessionScope.loginInfo.sessionUserId == boardMap.RegUserID}">
	        	<th class="ac">|&nbsp;<a href="javascript:actionForum('edit')">Edit</a></th>
	        	<th class="pdR10 ac">|&nbsp;<a href="javascript:actionForum('del')">DEL</a></th>
	        </c:if>
		</tr>
	     <tr>
	        <td>${boardMap.WriteUserNM}(${boardMap.LoginID})</td>
	        <td colspan="3" class="alignR pdR10">
	...
	<div class="alignBTN">
		...
		<c:choose>
			...
			<c:when test="${replyOption == '1' && ItemMgtUserMap.AuthorID == sessionScope.loginInfo.sessionUserId}">
				<span class="btn_pack medium icon"><span class="write"></span><input value="Reply" type="submit" id="make_new"></span>
			</c:when>
			<c:when test="${replyOption == '2' && BoardMgtInfo.MgtUserID == sessionScope.loginInfo.sessionUserId}">
				<span class="btn_pack medium icon"><span class="write"></span><input value="Reply" type="submit" id="make_new"></span>
			</c:when>
		</c:choose>
            ...
            <div class="cb_comment_area">
                <div class="cb_info_area">
                    ...
                    <c:if test="${sessionScope.loginInfo.sessionUserId == result.RegUserID}">
                        <span class="cb_nobar"><a href="javascript:editComment(${result.BoardID}, ${ItemID})">Edit</a></span>
                        <span class="cb_nobar"><a href="javascript:deleteComment(${result.BoardID}, ${ItemID})">Del</a></span>
                    </c:if>
                    </div>
                     <div id="dvd70reply_right" style="float:right;margin-top:8px;margin-right:90px;cursor:pointer;" >                  
