...
<div class="postInfo" id="mainBoardListArea">
    <div id="mainBoardLoading" style="padding:10px; text-align:center;"></div>
</div>
...
<script>
    ...
    function loadMainBoardList() {
        ...
        fetch("getData.do?" + params.toString(), { method: "GET" })
            ...
            .then(function(result) {
                renderMainBoardList(result.data || []);
            })
            .catch(function(error) {
                console.log("ERR: " + error.message);
                $("#mainBoardListArea").html("<div style='padding:10px;'>데이터를 불러올 수 없습니다.</div>");
            });
    }

    function renderMainBoardList(dataList) {
        ...
        if (!dataList || dataList.length === 0) {
            ...
            for (var i = 0; i < limit; i++) {
                ...
                html += '  <li style="width:65%;max-width:595px;">' + (item.Subject || "") + newImg + '</li>';
                html += '  <li style="width:20%;" class="alignC">' + (item.WriteUserNM || "") + '</li>';
                html += '  <li style="width:15%;" class="alignC">' + (item.ModDT2 || "") + '</li>';
                html += '</ul>';
            }
        }
        $("#mainBoardListArea").html(html);
    }
    ...
    function fnDetail(mgtID, ID) {
        ...
        fnOpenLayerPopup(url, data, "", 600, 650);
    }
</script>