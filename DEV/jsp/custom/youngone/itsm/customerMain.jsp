...
<link rel="stylesheet" type="text/css"
	href="${root}cmm/common/css/ui-layout.css" />
<script type="text/javascript" src="${root}cmm/js/dhtmlx/suite.js"></script>
<link rel="stylesheet" href="${root}cmm/js/dhtmlx/suite.css">
<script>
    
    async function setBoardNames(brdCodeHtmls) {
    	try {
    		const brdListRaw = await fetch("/olmapi/dic/category?category=BRDNM&languageID=${sessionScope.loginInfo.sessionCurrLangType}")
            const brdList = await brdListRaw.json();
    		const brdCodeNameMap = new Map(			// 처리속도 향상 위해 Map으로 변환
        			brdList.map(({ LABEL_CD, LABEL_NM }) => [LABEL_CD, LABEL_NM])
        	);      	
    		brdCodeHtmls.forEach(({ targetHtml, brdCode }) => {
   				const brdName = brdCodeNameMap.get(brdCode);
    			if (brdName) document.querySelector(targetHtml).textContent = brdName;
    		});
        	
    	} catch (err) {
    		console.error('setBoardNames 에러:', err)
    	}
    }
    ...
    $(document).ready(function(){
    	...
    	// 첫번째 ARC 탭: 요청한 업무
    	getArcName('ITS501010', "#arcTab01");
    
    	// 두번째 ARC 탭: 고객 테스트
    	getArcName('ITS501011', "#arcTab02");

    	// 세번째 ARC 탭: 만족도 조사
    	getArcName('ITS301011', "#arcTab03");

    	// 네번째 ARC 탭: 완료
    	getArcName('ITS502020', "#arcTab04");
    
    	// 우측 게시판 영역
    	const brdCodeHtmls = [
    		{targetHtml: '#boardTab01', brdCode: 'BRD3001'},		// FAQ
    		{targetHtml: '#boardTab02', brdCode: 'BRD3002'},	// 소프트웨어 자료
    		{targetHtml: '#boardTab03', brdCode: 'BRD3003'}		// 시스템 매뉴얼
    	]
    	
    	setBoardNames(brdCodeHtmls);
    	
    	 //btn
    	 if(clientID ==='0000000031' || clientID ==='0000000023'){
    		 getDicData("BTN", "ZLN0002").then(data => fnSetButton("remote", "", data.LABEL_NM));
    	 }
...
<body>
	<div class="main-container">
		<div class="section-wrapper" style="display: flex; gap: 30px;">
			<div class="left-side" style="flex: 1 0 auto; position: relative;">
				<ul class="tabs listTabs">
					<li id='arcTab01' onclick="loadTabPage(1)" class="tab"
						data-index="1"></li>
					<li id='arcTab02' onclick="loadTabPage(2)" class="tab"
						data-index="2"></li>
					<!-- <li id='arcTab03' onclick="loadTabPage(3)" class="tab" data-index="3"></li> -->
					<li id='arcTab04' onclick="loadTabPage(4)" class="tab"
						data-index="4"></li>
				</ul>
				
				<c:if test="${(sessionScope.loginInfo.sessionClientId eq '0000000031') or (sessionScope.loginInfo.sessionClientId eq '0000000023')}">
				<div class="btns"
					style="position: absolute; right: 0px; z-index: 999; top: 31px;">
					<button id="remote" onclick="fnCallLink('https://helpu.kr/yoit')"></button>
				</div>
				</c:if>
			...
			<div class="right-side">
				<ul class="tabs boardTabs">
					<li id='boardTab01' onclick="loadTabBoard(1)" class="tab board"
						data-index="1"></li>
					<!-- <li id='boardTab02' onclick="loadTabBoard(2)" class="tab board" data-index="2"></li>
		          <li id='boardTab03' onclick="loadTabBoard(3)" class="tab board" data-index="3"></li> -->
				</ul>
				<div class="border-section">
					<div class="pdT20 pdB10"></div>
	...
	<script>
	    ...
	    document.querySelector("#boardFrame").style.height = setWindowHeight() - 156+"px";
