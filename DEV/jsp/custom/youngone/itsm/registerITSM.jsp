...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		// button setting  		
  		...
  		// 02.srType
		fnSelect('srType', selectData + "&notInSRTypeList='WRK'", 'getSRTypeList', '${srType}' , 'Select', 'esm_SQL');
	...
	function fnGetCategory(srType){
		if(srType == ''){
			...
			fnSelect('category', selectData +"&srType=" + srType +"&level=1&customerNo=" + clientID, 'getESPSRCategory', '${category}', 'Select','esm_SQL');
		}
	}
...
<body>
	...
	<table class="form-column-2 new-form" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0"> 
		...
		<tr>
			...
			<td class="sline tit last">	
				<input type="text" id="reqdueDate" name="reqdueDate" value="" class="input_off datePicker stext" size="8" style="width: 70px;text-align: center;" onchange="this.value = makeDateType(this.value);"	maxlength="10">
		    </td>
		</tr>
		<tr>
			
			<!-- Process -->
			<th class="alignL pdL10">${menu.ZLN0134}<font color="red">&nbsp;*</font></th>
			<td class="sline tit last" style="position:relative;">
				<select id="srType" name="srType" class="sel" style="width:250px;">
