...
// 		ajaxSubmitNoAdd(document.langFrm, "templateSetting.do?templCode="+tmplCode, "blankFrame");
	...
	function getTreeSubItems(subItems){
	 ...
	 function clickMainMenu(menuType, menuName, menuIcon, defDimValueID, menuStyle, scndMenuStyle, menuUrl, boardMgtID, mainType, tmplFilter, projectID, tmplType, refreshYN, s_itemID, scrnMode, visitLogYN, MenuID){ 
		...
		}else if(menuType == "BOARD"){changeCheck=false; url ="boardMgtV4.do"; if($("#cmbTempl").val()=="LY004"){url="boardAdminMgt.do";} url = url+"?boardMgtID="+boardMgtID;name = "게시판";
...
<div id="maincontainer">
	...
	<div id="topsection" class="flex justify-between">
		<div class="align-center flex topL">
		  ...
		  <li class="align-center flex pdR10">
		  	<font onclick="fnChangePwd();" style="font-weight:bold;" class="cPointer">${sessionScope.loginInfo.sessionUserNm}</font>
			  <c:choose>
					<c:when test="${fn:length(userTeamList) >= 2}"> 
						 <form name="teamFrm" id="teamFrm" action="#" method="post" onsubmit="return false;">
			               		<ul id="nav">
			                   		<li class="top" style="padding: 4px 0;"><span id="teamName">(${sessionScope.loginInfo.sessionTeamName})</span>
		  ...
		  <c:if test="${loginIdx != 'BASE'}" > 
		  <li class="align-center flex pdR10">
		  	<font style="font-weight:bold;">${sessionScope.loginInfo.sessionUserNm}</font>
	 		 <c:choose>
				<c:when test="${fn:length(userTeamList) >= 2}">
					 <form name="teamFrm" id="teamFrm" action="#" method="post" onsubmit="return false;">
		               		<ul id="nav">
		  ...
		  <!-- 
		  <li class="align-center flex pdR10 pdT3">
		  	<font style="font-weight:bold;">${sessionScope.loginInfo.sessionUserNm}</font>
			<c:choose>
				<c:when test="${fn:length(userTeamList) >= 2}"> 
					 <div>
		   ...
		   -->
		  <li class="logout logoutbtn mgT1"><a href="#" class="logout" ><img src="${root}${HTML_IMG_DIR}/menu_icon_logout.png" style="cursor:pointer;display: block;"></a></li>       
       </div>
		<ul>
