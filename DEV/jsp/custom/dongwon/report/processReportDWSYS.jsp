...
try {
	...
	for (int totalCnt = 0; allTotalList.size() > totalCnt; totalCnt++) {
		...
		builder.getCellFormat().setWidth(200);
		builder.getParagraphFormat().setAlignment(ParagraphAlignment.CENTER);
		builder.write("동원시스템즈 ERP 구축");
		// builder.write(StringUtil.checkNullToBlank(titleItemMap.get("projectName")));	
		builder.insertCell();
		builder.getCellFormat().setWidth(100);
		builder.getParagraphFormat().setAlignment(ParagraphAlignment.CENTER);
	...
	builder.writeln("\t\t\t" + mainItemMap.get("toItemIdentifier") + " "
			+ mainItemMap.get("toItemName"));
		}
	}
		}
	} else {
	...
	builder.insertBreak(BreakType.SECTION_BREAK_NEW_PAGE);
		...
		name = StringUtil.checkNullToBlank(rowPrcData.get("ItemName"))
	...
	String AT00003 = StringUtil.checkNullToBlank(attrMap.get("AT00003")).replaceAll("&lt;", "<")
	.replaceAll("&gt;", ">").replaceAll("&quot;", "\"").replace("upload/", logoPath);
	...
	builder.getCellFormat().setVerticalAlignment(CellVerticalAlignment.CENTER);
	builder.getFont().setBold(true);
	builder.write(String.valueOf(AttrTypeList.get("AT00010"))); // ROLE
	...
	String AT00009 = StringUtil.checkNullToBlank(attrMap.get("AT00010")).replaceAll("&lt;", "<")
	.replaceAll("&gt;", ">").replaceAll("&quot;", "\"").replace("upload/", logoPath);
	...
	builder.getCellFormat().setVerticalAlignment(CellVerticalAlignment.CENTER);
	builder.getFont().setBold(true);
	builder.write(String.valueOf(AttrTypeList.get("AT00017"))); // 관련조직
	...
	String AT00037 = StringUtil.checkNullToBlank(attrMap.get("AT00017")).replaceAll("&lt;", "<")
	.replaceAll("&gt;", ">").replaceAll("&quot;", "\"").replace("upload/", logoPath);
	...
	builder.getCellFormat().setVerticalAlignment(CellVerticalAlignment.CENTER);
	builder.getFont().setBold(true);
	builder.write(String.valueOf(AttrTypeList.get("AT00030"))); // AS-IS
	...
	String AT00015 = StringUtil.checkNullToBlank(attrMap.get("AT00030")).replaceAll("&lt;", "<")
	.replaceAll("&gt;", ">").replaceAll("&quot;", "\"").replace("upload/", logoPath);
	...
	builder.getCellFormat().setVerticalAlignment(CellVerticalAlignment.CENTER);
	builder.getFont().setBold(true);
	builder.write(String.valueOf(AttrTypeList.get("AT00054"))); // TO-BE
	...
	String AT00016 = StringUtil.checkNullToBlank(attrMap.get("AT00054")).replaceAll("&lt;", "<")
	.replaceAll("&gt;", ">").replaceAll("&quot;", "\"").replace("upload/", logoPath);
	...
	String AT00026 = StringUtil.checkNullToBlank(attrMap.get("AT00026")).replaceAll("&lt;", "<")
	.replaceAll("&gt;", ">").replaceAll("&quot;", "\"").replace("upload/", logoPath);
	...
	if (elementList.size() > 0) {
		...
		builder.getCellFormat().getShading()
				.setBackgroundPatternColor(new Color(179, 179, 179));
		builder.getParagraphFormat().setAlignment(ParagraphAlignment.CENTER);
		builder.getCellFormat().setVerticalAlignment(CellVerticalAlignment.CENTER);
		builder.getFont().setSize(10);
		builder.getFont().setBold(true);
		...
		for (int j = 0; j < elementList.size(); j++) {
			...
			String itemName = StringUtil.checkNullToBlank(cnProcessData.get("Name"))
					.replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>", "");
			itemName = itemName.replace("&#10;", " ");
			itemName = itemName.replace("&#xa;", "");
			itemName = itemName.replace("&nbsp;", " ");
			String processInfo = StringUtil.checkNullToBlank(cnProcessData.get("Description"))
					.replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>", "")
	...
	builder.write(StringUtil.checkNullToBlank(data.get("ProcjectName")) + "/"
			+ StringUtil.checkNullToBlank(data.get("CSRName")));
	builder.insertCell();
	builder.getCellFormat().setWidth(100.0);
	builder.getParagraphFormat().setAlignment(ParagraphAlignment.CENTER);
	...
	for (int i = 0; i < relItemClassCodeList.size(); i++) {
		if (relItemClassCodeList.get(i).equals("CL07002")
				|| relItemClassCodeList.get(i).equals("CL08002")) {
			builder.insertBreak(BreakType.SECTION_BREAK_NEW_PAGE);
			break;
		}
	}
			...
			if (map.containsValue(relItemClassCodeList.get(i))) {
				...
				*/
				...
				if (ItemAttrInfo != null) {
					String AttrValue = StringEscapeUtils
							.unescapeHtml4(
									StringUtil.checkNullToBlank(ItemAttrInfo.get("AttrValue")))
							.replaceAll("&lt;", "<").replaceAll("&gt;", ">")
							.replaceAll("&quot;", "\"").replace("upload/", logoPath);
					if ("1".equals(StringUtil.checkNullToBlank(ItemAttrInfo.get("HTML")))) { // type이 HTML인 경우
						if (StringUtil.checkNullToBlank(ItemAttrInfo.get("AttrValue"))
						...
						builder.write(
								StringUtil.checkNullToBlank(ItemAttrInfo.get("LovValue"))); // Lov 값
					} else {
						builder.write(
								StringUtil.checkNullToBlank(ItemAttrInfo.get("AttrValue"))); // 해당 속성 내용
					}
				}
				builder.endRow();
				...
				if (ItemAttrInfo != null) {
					String AttrValue = StringUtil
							.checkNullToBlank(ItemAttrInfo.get("AttrValue"))
							.replaceAll("&lt;", "<").replaceAll("&gt;", ">")
							.replaceAll("&quot;", "\"").replace("upload/", logoPath);
					if ("1".equals(StringUtil.checkNullToBlank(ItemAttrInfo.get("HTML")))) { // type이 HTML인 경우
						if (StringUtil.checkNullToBlank(ItemAttrInfo.get("AttrValue"))
	...
	builder.insertHtml("<br>");
		...
		String imgPath = modelImgPath + modelMap.get("ModelID") + imgLang + "."
		+ GlobalVal.MODELING_DIGM_IMG_TYPE;
		...
		try {
	builder.insertHtml("<br>");
	builder.insertImage(imgPath, RelativeHorizontalPosition.PAGE, 30,
			RelativeVerticalPosition.PAGE, 20, width, height, WrapType.INLINE);
	builder.insertHtml("<br>");
		} catch (Exception ex) {
		}
	...
	//첨부문서 목록
	if (String.valueOf(request.getAttribute("fileYN")).equals("on") && !attachFileList.isEmpty()) {
		builder.insertBreak(BreakType.SECTION_BREAK_NEW_PAGE);
		builder.getFont().setColor(Color.DARK_GRAY);
		builder.getFont().setSize(11);
		builder.getFont().setBold(true);
	...
	//teamRole 목록
	if (String.valueOf(request.getAttribute("teamYN")).equals("on") && roleList != null
	&& !roleList.isEmpty()) {
		builder.insertBreak(BreakType.SECTION_BREAK_NEW_PAGE);
		builder.getFont().setColor(Color.DARK_GRAY);
		builder.getFont().setSize(11);
	...
	String itemName = StringUtil.checkNullToBlank(rowActData.get("ItemName"))
			.replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>", "");
	itemName = itemName.replace("&#10;", " ");
	itemName = itemName.replace("&#xa;", "");
	itemName = itemName.replace("&nbsp;", " ");
	...
	//20240828
		...
		String ATVITAT00003 = StringUtil.checkNullToBlank(rowActData.get("AT00008"))
				.replaceAll("&lt;", "<").replaceAll("&gt;", ">").replaceAll("&quot;", "\"")
				.replace("upload/", logoPath);
...
if (StringUtil.checkNullToBlank(rowActData.get("AT00029")).length() > 0){
	builder.getFont().setSize(11);
	builder.getFont().setBold(true);
	builder.writeln("[" + StringUtil.checkNullToBlank(AttrTypeList.get("AT00029")) + "]");
	builder.getFont().setSize(10);
	builder.getFont().setBold(false);
...
if (StringUtil.checkNullToBlank(rowActData.get("AT00053")).length() > 0){
	builder.getFont().setSize(11);
	builder.getFont().setBold(true);
	builder.writeln("[" + StringUtil.checkNullToBlank(AttrTypeList.get("AT00053")) + "]");
	builder.getFont().setSize(10);
	builder.getFont().setBold(false);
...
if (StringUtil.checkNullToBlank(rowActData.get("AT00010")).length() > 0){
	...
	if (0 != modelMap.size()) {
		...
		try {
	builder.insertHtml("<br>");
	builder.insertImage(imgPath, RelativeHorizontalPosition.PAGE, 30, RelativeVerticalPosition.PAGE, 20, width,
			height, WrapType.INLINE);
	builder.insertHtml("<br>");
		} catch (Exception ex) {
		}
...
//20240216       
String fileName;
String prefix = selectedItemPath != null && selectedItemPath.length() >= 11 ? selectedItemPath.substring(0, 11)
		: selectedItemPath;

fileName = prefix + "_BPD_" + itemNameOfFileNm + "_" + formatter.format(date)
		+ (outputType.equals("pdf") ? ".pdf" : ".docx");//
//~20240216
/*String fileName = firstTwelveChars + "BPD_" + itemNameOfFileNm + "_" + formatter.format(date) +".docx";
if(outputType.equals("pdf")){
	fileName = selectedItemPath + "BPD_" + itemNameOfFileNm + "_" + formatter.format(date) +".pdf";
}*/
