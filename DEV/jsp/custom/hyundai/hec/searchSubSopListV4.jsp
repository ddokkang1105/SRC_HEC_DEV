...
<script type="text/javascript">
	...
	var itemTypeCode = `${itemTypeCode}` || 'OJ00005';
	...
	function doDetail(avg){
		var url = "itemMainMgt.do?languageID=${sessionScope.loginInfo.sessionCurrLangType}"
				+"&s_itemID="+avg+"&scrnType=pop&screenMode=pop&accMode=${accMode}&itemMainPage=itm/itemInfo/itemMainMgt";
	...
	async function appendOption(){
		 ...
		 if("${defAttrTypeCode}" != ""){
			const defAttrTypeName = await getDefAttrTypeName();
	...
	function editCheckedAllItems(avg){ 
		...
		selectedCell.forEach(function(item) {
		    ...
		    seqLevelArr.push(item.SeqLevel);
		    authorIDArr.push(item.AuthorID); 
		...
		var SeqLevel = seqLevelArr.join(",");
		var AuthorID = authorIDArr.join(",");
		...
		if (items != "") {
			if (avg == "Attribute") {
				...
				Promise.all([
					...
					var afterConfirmCallbackFn = function() {
					...
					showDhxConfirm(rawText="Do you really subscibe this item?", afterConfirmCallbackFn, '', results[0].LABEL_NM , results[1].LABEL_NM )
				});
				...
				for(var j=0; j<items.length; j++){
					if(SeqLevel[j] > 1){
						...
						Promise.all([
							...
							showDhxAlert("보안등급으로 인해 리포트를 실행할 수 없습니다.", results[0].LABEL_NM);
						});
						return false;
				...
				if(item != ""){
					...
					var data = "itemInfoRptUrl=${itemInfoRptUrl}&pageUrl=/custom/hyundai/hec/report/zhecSubItemInfoRptPop";
    ...
    */
    ...
    /**
     * @function getDefAttrTypeName
     * @description api를 통해 defAttrType의 name 을 조회합니다.
     */
 	async function getDefAttrTypeName() {
    	const sqlID = "report_SQL.getAttrName";
 		const requestData = { sqlGridList : 'N' , sqlID , languageID, attrTypeCode : `${defAttrTypeCode}`};
 	    const params = new URLSearchParams(requestData).toString();
...
var pagination;
    ...
    grid = new dhx.Grid(null, {
        columns: [
            ...
            { hidden:true, id: "TeamName", header: [{ text: "${menu.LN00247}", align: "center" }], align: "left" },
            { width: 150,id: "L1Name", header: [{ text: "${menu.LN00353}", align: "center" }], align: "center" },
            { width: 150, id: "L2Name", header: [{ text: "${menu.LN00354}", align: "center" }], align: "center" },
            { flex: 1, id: "ItemName", header: [{ text: "SOP명", align: "center" }], align: "left" },
            { width: 180, id: "OwnerTeamName", header: [{ text: "${menu.LN00018}", align: "center" }], align: "center" },
            { hidden:true, width: 130, id: "Name", header: [{ text: "${menu.LN01007}", align: "center" }], align: "center" },
