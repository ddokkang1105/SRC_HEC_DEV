			...
			<div class="quick_menu" style="float:left !important">
				<ul>
					<c:forEach var="topMenu" items="${topMenuList}" varStatus="status" >
						<c:if test="${topMenu.MenuID ne 'MN007' && topMenu.MenuID ne 'MN015' }"> <!-- 문의 및 제안 미출력 -->
						...
						<c:if test="${topMenu.MenuID eq 'MN015' }"> <!-- 외부링크 팝업 -->
							<li class="topIcon" alt="${topMenu.MenuID}" title="챗봇" onclick="fnGoPop('${topMenu.URL}.do?${topMenu.FILTER}')"><img class="topIconImg" alt="img${topMenu.MenuID}" src="${root}${HTML_IMG_DIR_SHORTCUT}${topMenu.Icon}"></img></li>
						</c:if>
					</c:forEach>
				</ul>
			</div>
