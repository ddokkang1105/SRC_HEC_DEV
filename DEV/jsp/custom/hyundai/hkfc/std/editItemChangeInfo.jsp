...
<script type="text/javascript">
	...
	function saveChangeSetInfo() {
		...
		if(confirm("${CM00023}")){ 			
			var url = "admin/saveNewChangeSet.do";
	...
	function fnCheckInItem() {
		...
		//if(confirm("${CM00049}")){	// 해당 항목의 변경을 완료 하시겠습니까?
			fetch("checkInPassCheck.do?itemID=${s_itemID}&languageID=${sessionScope.loginInfo.sessionCurrLangType}&sessionCurrLangCode=${sessionScope.loginInfo.sessionCurrLangCode}&cngts=${getData.ChangeSetID}")
			.then(res => res.json())
			.then(res => {
				if(res.message) {
					alert(res.message);
		...
		<table style="table-layout:fixed;" class="tbl_blue01 mgT10">
			...
			<tr>
				...
				<td  class="viewtop last">
				 ${getData.Version}
					<input type="hidden" id="version" name="version" value="${getData.Version}" class="text" style="width:100%">
				</td>
			...
			<tr>	
				...
				<td class="last alignL">
				 <input type="text" id="ValidFrom" name="ValidFrom" value="${getData.ValidFrom}"	class="input_off datePicker stext" size="8"
