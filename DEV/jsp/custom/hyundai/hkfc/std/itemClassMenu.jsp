...
// 			}
					...
					.then(data => {
						...
						if(info.size < 2) {  // 팝업 없이 호출 
							...
							fetch("checkOutItem.do", {
								...
								.then(res => res.text()) // 서버에서 script 형태 응답 시
								.then(() => {
								  alert("${WM00067}"); // 저장되었습니다
								  fnItemMenuReload();  // 직접 호출
								}).catch(err => {
	...
	function goApprovalPop() {
		 ...
		 window.open("" ,"${wfURL}", "width=1200, height=830, left=400, top=200"); 
		 wfFrame.action =url;
		 wfFrame.method="post";
	...
	function editAuthor(){
		...
		var h = 350;
		var url = "selectOwnerPop.do";
		var data = "items=${s_itemID}&hideOption=N&isMail=Y";
		    ...
		    <c:if test="${scrnType ne 'pop' && accMode ne 'OPS' && (sessionScope.loginInfo.sessionUserId eq itemInfo.AuthorID || sessionScope.loginInfo.sessionAuthLev eq '1')  && itemInfo.ChangeMgt eq '1'}" >	
			   ...
			   <!-- Check In  Only Check In Option = 01 , 02, 03 03B -->
				...
				<c:if test="${itemInfo.Blocked eq '0' && itemInfo.CheckInOption eq '03A'}" >
				   	<span class="btn_pack small icon"><span class="checkin"></span><input value="Check In" type="button" onclick="fnCheckInItem()"></span>
				</c:if>
			   	<!-- 변경 담당자 Rework -->				       
				...
				<c:if test="${itemInfo.WFInstanceID != ''  && itemInfo.WFStatus eq '1' && itemInfo.CSStatus == 'APRV' && wfInstanceON eq 'N' }" >
				  &nbsp;<span class="btn_pack medium icon"><span class="confirm"></span><input value="Withdraw Check In" onclick="withdrawAprvReq()" type="submit"></span> 					     
				</c:if>      		
			</c:if>
