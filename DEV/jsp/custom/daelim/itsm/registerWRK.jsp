...
<script type="text/javascript">
	...
	<c:if test="${startEventCode ne 'WRK0001' }">
	...
	jQuery(document).ready(function() {
		...
		fnSelectSetting("Y");
  		...
  		if("${startEventCode}" !== "WRK0001") fnEspMbrRcdListFrame();
  		
  		// 완료예정일
  		<c:if test="${startEventCode eq 'WRK0001' }">
	...
	function fnCheckValidation(temp){
		...
		<c:if test="${startEventCode eq 'WRK0001' }">
			...
			if(description == "" ){ alert("${WM00034_2}"); tinyMCE.get('description').focus(); isCheck = false; return isCheck;}
		</c:if>
		
		if(temp === undefined || temp === '' || temp === null){
			<c:if test="${startEventCode eq 'WRK0001' }">
				var SRAT0069 = document.querySelector("#SRAT0069");
				if(SRAT0069 == ""){ alert("${WM00034_5}"); SRAT0069.focus(); isCheck = false; return isCheck;}
			</c:if>
			...
			if(category == "" && subCategory == "") $("#defCategory").val($("#category option:eq(1)").val());
		}
	...
	// 운영실적 SAVE 
	 function callChildSaveMH(srID) {
		 if("${startEventCode}" !== "WRK0001") {
        	childWindow.saveMH(srID,"WRK0003","WGRWRK100");
		 }
    }
	...
	function fnEspMbrRcdListFrame(){
		...
		var data = "docCategory=WKS&speCode=${startEventCode}";
	...
	function fnSRAreaLoad() {
		fetch("/olmapi/srArea/?languageID=${sessionScope.loginInfo.sessionCurrLangType}&userID=${sessionScope.loginInfo.sessionUserId}&myCSR=${myCSR}")
...
<body>
	...
	<div class="page-title btn-wrap">
		<c:choose>
			<c:when test="${startEventCode eq 'WRK0001' }">내부요청</c:when>
			<c:otherwise>작업등록</c:otherwise>
		</c:choose>
	</div>
	...
	<table class="form-column-2 new-form" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0"> 
		...
		<tr>
			...
			<c:if test="${startEventCode eq 'WRK0001' }">
			<!-- 완료예정일 -->
			<th class="alignL pdL10">완료예정일<font color="red">&nbsp;*</font></th>
			<td class="sline tit last" >
				<input type="text" id="SRAT0069" name="SRAT0069" class="text datePicker stext" size="8" autocomplete="off"
		...
		<c:choose>
			<c:when test="${startEventCode eq 'WRK0001' }">
			<tr>
			...
			<c:otherwise>
			<tr>
				<th class="alignL pdL10">처리내용<font color="red">&nbsp;*</font></th><!-- 처리내용 -->
				<td style="height:300px;" class="sline tit last" colspan="3">
