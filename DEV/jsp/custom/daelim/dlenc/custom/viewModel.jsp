...
<head>
	...
	<script type="text/javascript">
		...
		var mdlFilter = "${mdlFilter}";
		var itemTypeFilter = encodeURIComponent("${itemTypeFilter}");
		var dimTypeFilter = encodeURIComponent("${dimTypeFilter}");
		var symTypeFilter = encodeURIComponent("${symTypeFilter}");
		...
		var msLanguageType = "${languageID}";
		if(msLanguageType == ""){
			 msLanguageType = "${sessionScope.loginInfo.sessionCurrLangType}";
		}
...
function perView75(graph) {
	// 페이지 관련 설정 제거
	graph.pageVisible = false;
	graph.pageScale = 1;
	
	// 줌 비율 설정
	graph.view.setScale(0.75);
	
	// 스크롤 중앙 정렬
	var pad = graph.getPagePadding();
	var scale = graph.view.scale;
	
	graph.container.scrollTop = pad.y * scale;
	...
	// 디버깅 로그
	// console.log("줌 비율:", scale, " / pageScale:", graph.pageScale);
}
(function()
{
	...
	mxUtils.getAll([bundle, STYLE_PATH + '/default.xml', STYLE_PATH + '/default-old.xml'], function(xhr)
	{
		...
		if(mdlFilter == "Y"){			
			var opacityList = "${opacityList}";			
			var itemTypeCodeList = "${itemTypeFilter}";
			var blurredList = "";
			
			var param = "opacityList="+opacityList
			+ "&itemTypeCodeList="+itemTypeCodeList
			+ "&blurredList="+blurredList
			+ "&s_itemID=${ItemID}"
			+ "&modelID=${ModelID}"
			+ "&option=${option}"
			+ "&tLink=${tLink}"
			...
			$.ajax({
				url:"getDiagramXml.do",
				type:"POST",
				data:param,
				contentType: "application/x-www-form-urlencoded; charset=UTF-8",
				success: function(result){
					fnCallbackGetDiagramXml(result);
		...
		if((focusedItemID == null || focusedItemID == "" || focusedItemID == undefined) && (zoomOption != "" || zoomOption != null)){
			if(zoomOption == "FTP"){fnGoFitPage(graph);}else if(zoomOption == "FTW"){ ui.fitWindow(graph);}else if(zoomOption == "PV75"){ perView75(graph);}
		}
		...
		// focusedItemID 선택되는 부분
		if(focusedItemID != null || focusedItemID != "" || focusedItemID != undefined) {
			var a  = ui.editor.getGraphXml();
			var b = a.getElementsByTagName("UserObject");
			var id = Array.prototype.filter.call(b, (e) => e.getAttribute("link") === focusedItemID)[0].id;
			graph.addSelectionCell(graph.getModel().getCell(id));
		}
...
function fnOpenSymbols(){
    var itemID = "${ItemID}"; 
    var url = "goViewSymbolPop.do?itemID=" + itemID;
    var w = "830";
    var h = "680";
...
function fnReloadModelFilter(opacityList, itemTypeCodeList, blurredList, dimTypeID, dimValueList, symTypeCodeList) {   
  ...
  function fnCallbackGetDiagramXml(newXML){
	var splitResult = newXML.split("###");
	var diagramForXml = splitResult[0];
	var updateForXml = splitResult[1];
	
    var orgdoc = mxUtils.parseXml(diagramForXml);
	ui.editor.setGraphXml(orgdoc.documentElement);
	
	updateProcess(updateForXml);
