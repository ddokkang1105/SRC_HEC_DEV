...
// 		autoComplete("mainSearchValue", "AT00001", "", "", "", 5, "top");
	...
	function changeTempl(layerType,layerText, mainUrl, mainScnText, mainUrlFilter, tmplFilter, tmplProjectID, tmplType, arcLinkYN, mainType) {
		if(layerText) {
			let global = "<%=GlobalVal.HTML_TITLE%>";
			window.parent.document.title=global+" ("+layerText+")";
		}
...
// 			defDimValueID = "${arcLinkDimTypeID}";
			...
			if(menuType === "process") {
				if("${templCode}" == "HQ00001") { // 본사/서산
					const result  = findMatchingEntry("${templCode}", "GSDM01HQ");
					if(result.found) {
						menuType = "GSDM01HQ";
						mainScnText = result.MENU_NM;
						mainUrl = result.URL+".do?"+result.FILTER;
						menuStyle = result.STYLE;
					} else {
						menuType = "TMPL_CHG_URL";
						mainScnText = $("#mainScnTextHmBtn").val();
						mainUrl = $("#mainUrlHmBtn").val()+".do?"+$("#mainUrlFilterHmBtn").val();
						menuStyle = "";
					}
				}
				if("${templCode}" == "BAEX001") { // SKBA
					const result  = findMatchingEntry("${templCode}", "GSDM01BA");
					if(result.found) {
						menuType = "GSDM01BA";
						mainScnText = result.MENU_NM;
						mainUrl = result.URL+".do?"+result.FILTER;
				...
				if("${templCode}" == "OS00001") { // SKOS
					menuType = "TMPL_CHG_URL";
					mainScnText = mainScnText;
					mainUrl = mainUrl;
					menuStyle = "";
				}
			}
			
			else if(menuType === "tech") {
				if("${templCode}" == "HQ00001") { // 본사/서산
					const result  = findMatchingEntry("${templCode}", "GSDM02HQ");
					if(result.found) {
						menuType = "GSDM02HQ";
						mainScnText = result.MENU_NM;
						mainUrl = result.URL+".do?"+result.FILTER;
				...
				if("${templCode}" == "BAEX001") { // SKBA
					const result  = findMatchingEntry("${templCode}", "GSDM02BA");
					if(result.found) {
						menuType = "GSDM02BA";
						mainScnText = result.MENU_NM;
						mainUrl = result.URL+".do?"+result.FILTER;
			...
			else if(menuType === "rule") {
				if("${templCode}" == "HQ00001") { // 본사/서산
					const result  = findMatchingEntry("${templCode}", "GSDM04HQ");
					if(result.found) {
						menuType = "GSDM04HQ";
						mainScnText = result.MENU_NM;
						mainUrl = result.URL+".do?"+result.FILTER;
				...
				if("${templCode}" == "BAEX001") { // SKBA
					const result  = findMatchingEntry("${templCode}", "GSDM04BA");
					if(result.found) {
						menuType = "GSDM04BA";
						mainScnText = result.MENU_NM;
						mainUrl = result.URL+".do?"+result.FILTER;
				...
				if("${templCode}" == "OS00001") { // SKOS
					const result  = findMatchingEntry("${templCode}", "GSDM04OS");
					if(result.found) {
						menuType = "GSDM04OS";
						mainScnText = result.MENU_NM;
						mainUrl = result.URL+".do?"+result.FILTER;
			...
			else if(menuType === "term") {
				const result  = findMatchingEntry("${templCode}", "GSDM06", true);
				if(result.found) {
					menuType = "GSDM06";
					mainScnText = result.MENU_NM;
					mainUrl = result.URL+".do?"+result.FILTER;
			...
			else if(menuType === "ter") { //기술문서
			    const result = findMatchingEntry("${templCode}", "PAL14"); //SKONDEV 기준 ArcCode
			    if (result.found) {                
			      mainScnText = result.MENU_NM;      
			      menuStyle = result.STYLE;
			      
			      const linkUrl = "${linkUrl}" || "";
			      const itemMatch = linkUrl.match(/itemID=([^?&]+)/i);
			      const itemID = itemMatch ? itemMatch[1] : "";
			      
		          if (linkUrl && itemID) {
		              fetch("getItemAccRight.do?itemID=" + itemID + "&userID=${sessionScope.loginInfo.sessionUserId}")
		                  .then(res => res.json())
		                  .then(data => {
		                      if (data.data === "Y") {
		    			          clickMainMenu("PAL14", mainScnText, "", "", menuStyle, "", result.URL + ".do?" + result.FILTER + "&linkUrl=" + encodeURIComponent(linkUrl));
		                          );
		                      } else {
		                          alert("이 문서를 볼 수 있는 권한이 없습니다."); //nct 경고문구 가져오거나 새로 만들기
		                          menuType = "TMPL_CHG_URL";
		        			      mainScnText = $("#mainScnTextHmBtn").val();
		        			      mainUrl = $("#mainUrlHmBtn").val() + ".do?" + $("#mainUrlFilterHmBtn").val();
		        			      menuStyle = "";
		                  ...
		                  .catch(err => console.error("권한 확인 오류:", err));
		              return;
		          }  else {
			      menuType = "TMPL_CHG_URL";
			      mainScnText = $("#mainScnTextHmBtn").val();
	...
	function findMatchingEntry(templCode, menuId, mainMenu) {
		const items = mainMenu ? "mainMenuList".match(/{[\s\S]*?}/g) : "${scnMenuList}".match(/{[\s\S]*?}/g); // 줄바꿈 포함한 각 객체 추출
		if (!items) return { found: false };
		
		for (const item of items) {
			const templMatch = item.match(/TemplCode=([^\s,}]+)/);
			const menuMatch = item.match(/MENU_ID=([^\s,}]+)/);
		
			if (templMatch?.[1] === templCode && menuMatch?.[1] === menuId) {
				// 원하는 필드 추출
				const menuNmMatch = item.match(/MENU_NM=([^\s,}]+)/);
				const urlMatch = item.match(/URL=([^\s,}]+)/);
				const filterMatch = item.match(/FILTER=([^\n,}]*)/);
				const styleMatch = item.match(/STYLE=([^\s,}]+)/);
				
				return {
					found: true,
					MENU_NM: menuNmMatch?.[1] || null,
					URL: urlMatch?.[1] || null,
					FILTER: filterMatch?.[1] || null,
	...
	var selectedText = "";
	function fnOpenPapagoPopup() {
		 selectedText = localStorage.getItem("selectedText");
		 if(selectedText == null) selectedText = "";
	
		 popup("getPapagoTrans.do?text="+encodeURIComponent(selectedText), '<img src="${root}${HTML_IMG_DIR}icon_trans.png" class="mgR6" style="width: 25px;"/></li>번역하기', 1200, 420);
	}
	
	window.addEventListener("message", (event) => {
	    if (event.data.type === "selectedText") {
	        console.log("선택한 텍스트:", event.data.text);
	        localStorage.setItem("selectedText", event.data.text);
	    }
	});
</script>
</head>
<body>
<form name ="papagoTrans" >
...
<div id="maincontainer">
	...
	<div id="topsection" class="flex justify-between">
		<div class="topL">
		  ...
		  <li class="pdR10 pdT6 cPointer" OnClick="fnChangePwd();" alt="USER" titl="USER"> <font style="font-weight:bold;"><u>${sessionScope.loginInfo.sessionUserNm}</u></font>&nbsp;(${sessionScope.loginInfo.sessionTeamName})</li> 
		  </c:if>
		  <c:if test="${loginIdx != 'BASE'}" > 
		...
		<div class="floatR pdR0" id="MemInfo">
			<ul class="floatR">
				<li class="pdR10 pdL20 mgT5"><button onclick="fnOpenPapagoPopup()" class="translate">번역</button></li>
								...
								<c:forEach var="mainMenu" items="${mainMenuList}" varStatus="status" >
									<c:if test="${templ.TemplCode == mainMenu.TemplCode}">
										<c:choose>
												...
												<ul>
													<c:forEach var="scnMenu" items="${scnMenuList}" varStatus="status" >
														<c:if test="${mainMenu.MENU_ID == scnMenu.PRNT_MENU_ID && mainMenu.TemplCode == scnMenu.TemplCode}">
																	...
																	<ul>
																		<c:forEach var="thdMenu" items="${thdMenuList}" varStatus="status" >
																			<c:if test="${scnMenu.MENU_ID == thdMenu.PRNT_MENU_ID}">
																				<li onclick="clickMainMenu('${thdMenu.PRNT_MENU_ID}','${mainMenu.MENU_NM}','${mainMenu.ICON}', '${thdMenu.DefDimValueID}','${mainMenu.STYLE}','${scnMenu.STYLE}','${thdMenu.URL}.do?${thdMenu.FILTER}','','','','','','','','','Y','${thdMenu.MenuID}')"><a href="#" alt="${thdMenu.MENU_NM}">${thdMenu.MENU_NM}</a></li>
																			</c:if>
																		</c:forEach>
																	</ul>
																</li>
															...
															<c:if test="${scnMenu.DimTypeID == '0'}">
																<li onclick="clickMainMenu('${scnMenu.MENU_ID}','${mainMenu.MENU_NM}&nbsp;::&nbsp;${scnMenu.MENU_NM}','${mainMenu.ICON}','','${mainMenu.STYLE}','${scnMenu.STYLE}','${scnMenu.URL}.do?${scnMenu.FILTER}','','','','','','','','','Y','${scnMenu.MenuID}')"><a href="#" alt="${scnMenu.MENU_NM}">${scnMenu.MENU_NM}</a></li>
															</c:if>
														</c:if>
													</c:forEach>	
												</ul>
											...
											<c:otherwise>
							         			<li onclick="clickMainMenu('${mainMenu.MENU_ID}','${mainMenu.MENU_NM}','${mainMenu.ICON}','','${mainMenu.STYLE}','','${mainMenu.URL}.do?${mainMenu.FILTER}','','','','','','','','','Y','${mainMenu.MenuID}')"><a href="#" alt="${mainMenu.MENU_NM}" class="topmenu" id="${mainMenu.MENU_NM}"><c:if test="${mainMenu.ICON != ''}"><img class="menuicon" src="${root}${HTML_IMG_DIR_ARC}/${mainMenu.ICON}"></img></c:if>${mainMenu.MENU_NM}</a></li>
											</c:otherwise>
										</c:choose>
									</c:if>
								</c:forEach>
