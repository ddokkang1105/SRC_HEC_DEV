...
<script>
//scrnMode 변수 선언
	var scrnMode = "${scrnMode}"; // "E"가 아니면 checkbox 컬럼을 숨길 수 있도록 설정
	...
	var grid = new dhx.Grid("grid",  {
		columns: [
	        ...
	        { width: 30, id: "checkbox", header: [{ text: "<input type='checkbox' onclick='fnMasterChk(checked)'></input>" }]
		        , align: "center", type: "boolean"
		        , editable: true, sortable: false , htmlEnable: true
		        ,hidden: scrnMode !== "E"  },
	        ...
	        { 
	            ...
	            id: "Name", 
	            header: [
	                { text: "${menu.LN00004}", align: "center" }
	                ,...(scrnMode === "E" ? [{ content: "inputFilter" }] : [])
	            ]
	        },
	        { width: 150, id: "AssignmentTypeName"
	        	, header: [{ text: "Role Type", align:"center" }
	        	, ...(scrnMode === "E" ? [{ content: "selectFilter" }] : [])
	        ]},
	        { width:110, id: "RoleTypeTxt"
	        	, header: [{text: "Role", align:"center"}
	        	, ...(scrnMode === "E" ? [{ content: "selectFilter" }] : [])
	        ]},
	        { width: 80, id: "AccessRightName"
	        	, header: [{ text: "${menu.LN00149}", align:"center" }
	        	,  ...(scrnMode === "E" ? [{ content: "selectFilter" }] : [])], align:"center"},
	        { id: "Path"
	          , header: [{ text: "${menu.LN00043}", align:"center" }
	          ,  ...(scrnMode === "E" ? [{ content: "selectFilter" }] : [])
	          ], align:"left" },
	        { width: 110, id: "AssignedDate"
	        	  , header: [{ text: "${menu.LN00078}", align:"center" }
	        	  ,  ...(scrnMode === "E" ? [{ content: "selectFilter" }] : [])]
	          	  , align:"center"},
	        { width:70, id: "OrderNum", header: [{ text: "Order", align:"center" }
	        		, ...(scrnMode === "E" ? [{ content: "selectFilter" }] : [])], align:"center", htmlEnable: true},
	        { 
	        		    width: 80, 
	        		    id: "Assignment", 
	        		    header: [
	...
	grid.events.on("cellClick", function(row,column,e){
		if(column.id != "checkbox"&&scrnMode === 'E'){
