			...
			<div id="process" class="mgB10">
				...
				<table class="tbl_preview mgB10">
					...
					<tr>
						<!-- 문서번호 -->
						<th>${menu.ZLN0068}</th>
						<td colspan="1" class="alignL pdL10" id="ZLN0068">${itemInfo.Identifier}</td>
						<!-- 문서명 -->
						...
						<!-- 개정번호 -->
						<th>${menu.LN00356}</th>
					    <td  colspan="4" class="alignL pdL10" id="LN00356">	${prcList.Version}<span style="font-weight:bold; margin-left:5px;">(${csInfo.CSStatusName})</span></td>  
					</tr>
					<tr>
						<!-- 담당Site -->
					    <th>${menu.ZLN0089}</th>
						...
						<!-- 담당부서 -->				
						<th>${menu.ZLN0074}</th>
						<td  colspan="1" class="alignL pdL10" id="ZLN0074">	${itemInfo.OwnerTeamName}</td>
						<!-- 담당자 -->
						<th>${menu.LN00004}</th>
						<td  class="alignL pdL10" id="LN00004">${itemInfo.AuthorName}</td>
						<!-- NCT대상 -->
						...
						<td class="alignL pdL10 alignt" colspan="10" style="width:100%;height:200px;">							
							...
							<div class="tmp_file_wrapper mgT10" id="tmp_file_wrapper">
								<table class="tbl_preview"  id="tmp_file_items" name="tmp_file_items">
									...
									<tbody name="file-list" id="fileListTable"></tbody>
								</table>
							</div>
						</td>	
					</tr>
...
/* 	function popStandardTemrs() {
	...
	async function renderBasicInfoSetting(s_itemID) {
	    ...
	    // --- 1. itemInfo 관련 데이터 할당 ---
	    // 문서번호
	    document.getElementById('ZLN0068').textContent = itemInfo.Identifier || '';
	    
	    // 문서명
	    document.getElementById('AT00001').value = itemInfo.ItemName || '';
	    
	    // 담당부서
	    document.getElementById('ZLN0074').textContent = itemInfo.OwnerTeamName || '';

	    // 담당자
	    document.getElementById('LN00004').textContent = itemInfo.AuthorName || '';
		
	    /*
	    // cs version
	    document.getElementById('LN00356').innerHTML = `
            \${prcList.Version || ''}
            <span style="font-weight:bold; margin-left:5px;">(\${csInfo.CSStatusName || ''})</span>
        `;

	    // 주요개정사항
	    document.getElementById('Description').value = csInfo.Description || '';
	    // 문서개요
	    document.getElementById('AT00003').value = prcList.Description || '';
	...
	async function renderItemNameAndPath(info){
		...
		html = ` <img src="${root}${HTML_IMG_DIR_ITEM}/\${info.ItemTypeImg}" OnClick="fnOpenParentItemPop('${parentItemID}');" style="cursor:pointer;">
				  <font color="#3333FF"><b style="font-size:13px;">\${info.Identifier}&nbsp;\${info.ItemName}</b></font>&nbsp;	
  	...
  	*/
  	async function renderFileListByOption() {
  		...
  		try{
  			...
  			const fileFormat = fileList.FileFormat || '';
  		    let iconClass = 'log'; // 기본값

  		    // 파일 포맷에 따른 아이콘 클래스 분기 (JSTL의 c:choose 로직)
  		    ...
  		    // HTML 템플릿 생성
  		    table.innerHTML = fileList.map(list => `
  		        <tr id="\${list.Seq}">
  		            <td>
  		                <svg xmlns="http://www.w3.org/2000/svg" onclick="fnDeleteFile('\${list.Seq}')" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368">
  		            ...
  		            <td class="alignL pdL10 flex align-center">
  		                ...
  		                <span style="cursor:pointer;margin-left: 10px;">\${list.FileRealName}</span>
  		                (<span id="fileSize">\${list.FileSize}</span>)
  		            </td>
