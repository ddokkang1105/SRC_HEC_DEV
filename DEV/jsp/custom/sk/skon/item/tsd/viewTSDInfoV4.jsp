...
async function getDefSKONAttr(callback){
		
		// 기본 html 렌더링
		await renderItemInfo();
...
//			//window.open(url,1316,h); 
	...
	//===============================================================================
		   ...
		   async function updateCheckboxes() {
			   ...
			   var dimResult = await getDimensionList();
			   var dimData = processDimensionData(dimResult);
			   var dimList = dimData.dimList;
		...
		// dimension 리스트 조회
		   ...
		   function processDimensionData(dimInfoList) {
		       let dimResultList = [];
		       let dimResultMap = {};
		       let dimInfotMap = {}; 
		       ...
		       if (!dimInfoList || dimInfoList.length === 0) return { dimResultList: [], dimList: [] };

		       dimInfoList.forEach((map, i) => {
		           const dimValueId = String(map.DimValueID || "").trim();
		           if (dimValueId) {
		               dimInfotMap[dimValueId] = dimValueId;
		           }

		           const currentTypeName = String(map.DimTypeName || "");
		           const currentValuePath = String(map.DimValuePath || "");

		           if (i > 0) {
		               if (dimTypeName === currentTypeName) {
		                   dimValueNames += " / " + currentValuePath;
		               } else {
		                   dimResultMap["dimValueNames"] = dimValueNames;
		                   dimResultList.push(dimResultMap);
		                   
		                   dimResultMap = {}; 
		                   dimTypeName = currentTypeName;
		                   dimResultMap["dimTypeName"] = dimTypeName;
		                   dimValueNames = currentValuePath;
		               }
		           } else {
		               dimTypeName = currentTypeName;
		       ...
		       if (dimInfoList.length > 0) {
		           dimResultMap["dimValueNames"] = dimValueNames;
		           dimResultList.push(dimResultMap);
		       }

		       var dimList = Object.keys(dimInfotMap);

		       return {
		           dimResultList: dimResultList, 
		           dimList: dimList,            
		           dimInfotMap: dimInfotMap  
		       };
		   }
	...
	function fnEditItemInfo() {
		...
		+"&itemEditPage=custom/sk/skon/item/tsd/editTSDInfoV4"
    ...
    * @returns {String} 파일 옵션
	...
	/**
   	* @function renderFileListByOption
  	...
  	async function renderFileListByOption() {
  		...
  		try{
  			...
  			container.innerHTML = fileList.map(list => `
  			  ...
  			  <td class="alignL pdL10">\${list.AuthorName ?? ""}</td>
  ...
  //itemInfo 조회
  	async function getCSInfo() {
  		const sqlID = "cs_SQL.getChangeSetInfo";
  		const sqlGridList = 'N';
  		const requestData = { languageID, s_itemID, accMode, changeSetID, sqlID, sqlGridList };
  		const params = new URLSearchParams(requestData).toString();
  		const url = "getData.do?" + params;
  	...
  	async function renderItemInfo() {
  		...
  		try {
  	        ...
  	        if (!itemInfo || itemInfo.length === 0) {
  	      	...
  	      	const csInfoArr = await getCSInfo();
  	      	const csInfo = csInfoArr[0];
	        ...
	        console.log('csInfo');
	        console.log(csInfo);
	        console.log(`\${csInfo.CSStatusName}`);
  			...
  			tbody.innerHTML = `
				...
				<!-- 문서명 -->
				<th>${menu.LN00101}</th>
			    <td colspan="3" class="alignL pdL10">\${itemInfo.ItemName}</td>				
				<!-- 개정번호 -->
				<th>${menu.LN00356}</th>
				<td colspan="4" class="alignL pdL10">\${itemInfo.Version}<span style="font-weight:bold; margin-left:5px;">(\${csInfo.CSStatusName || ''})</span></td>
			</tr>
			...
			<tr>
				...
				<td colspan="2" class="alignL pdL10"  style="display:none;">\${itemInfo.ProjectName}</td>
			</tr>
			...
			<tr>
				...
				<textarea  id="description" name="description"  style="height:100px;width:100%;" readonly="readonly">\${itemInfo.Description}</textarea>
				</td>
			</tr>
			<!-- 주요개정사항 -->
			<tr>
			...
			<tr>
				<th>${menu.ZLN0070}</th>
				<td class="alignL pdL10" colspan="10"></td>		
			</tr>
  			`;
