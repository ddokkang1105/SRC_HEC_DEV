...
// 			}
	...
	function goApprovalPop() {	
		...
		var url = "${wfURL}.do?";
		var data="isPop=Y&changeSetID=${itemInfo.CurChangeSet}&isMulti=N&wfDocType=CS&docSubClass=${itemInfo.ClassCode}";
	...
	function editAuthor(){
		...
		var h = 350;
		var url = "selectOwnerPop.do";
		var data = "items=${s_itemID}&hideOption=Y";
	...
	<ul>
		...
		<div id="functions">
	       ...
	       <c:if test="${itemInfo.ChangeMgt eq '1'}">
		  	    <span class="btn_pack small icon" style="display:none;"><span class="cs"></span><input value="History" type="button" onclick="fnGoChangeMgt()"></span>			
		    ...
		    <c:if test="${scrnType ne 'pop' && accMode ne 'OPS' && (sessionScope.loginInfo.sessionUserId eq itemInfo.AuthorID || sessionScope.loginInfo.sessionAuthLev eq '1')  && itemInfo.ChangeMgt eq '1'}" >	
	   		...
	   		<span class="btn_pack medium icon"><span class="reload"></span><input value="Reload" type="button" onclick="fnMenuReload()"></span>
	   	 	<c:if test="${checkItemAuthorTransferable eq 'Y'}" >
	   			<span class="btn_pack small icon"><span class="gov"></span><input value="Take Over" type="button" onclick="fnTransferItem()"></span>
	   		</c:if>
	   		<c:if test="${myItem eq 'Y' || sessionScope.loginInfo.sessionAuthLev eq '1'}" >
