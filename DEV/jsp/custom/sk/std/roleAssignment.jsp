...
<spring:message code="${sessionScope.loginInfo.sessionCurrLangCode}.WM00190" var="WM00190"/>
...
<script>	
	...
	function fnChangeAuthor() {
		...
		if(selectedCell.length > 1){
			alert("${WM00190}");		// 한 개만 선택해주세요.
			return false;
		}
		
		$("#AuthorID").val(selectedCell[0].MemberID);
...
<form name="roleFrm" id="roleFrm" action="" method="post" onsubmit="return false;">
	...
	<input type="hidden" id="checkTransfer" name="checkTransfer" value="1" />
	...
	<div class="countList" style="margin: 3px 0px 3px 0;" >
        ...
        <li class="floatR">
        	<span class="btn_pack nobg"><a class="gov" onclick="fnChangeAuthor()" title="Ownership"></a></span>
	...
	<table class="tbl_blue01 mgT10 mgB5" style="table-layout:fixed;" width="100%" cellpadding="0" cellspacing="0">
		...
		<tr>
			<!-- 담당자 / 우선순위 / R&R / 권한 / 우선순위 / Active -->
			<th class="last pdL10">${menu.LN00004}</th>		
			<th class="last pdL10">${menu.LN00067}</th>
			<th class="last pdL10" style="display:none;">${menu.LN00163}</th>	
			<th class="last pdL10" style="display:none;">${menu.LN00149}</th>	
			<th class="last pdL10" style="display:none;">${menu.LN00067}</th>	
			<th class="last pdL10">${menu.ZLN0143}</th>	
		</tr>
		<tr>
			<td class="last">
		...
		<tr>
			<th class="last pdL10" style="display:none;">Task</th>				
			<td class="last" colspan=5 align="left" style="display:none;">
...
<script>
	...
	var grid = new dhx.Grid("grid",  {
	    columns: [
	    	// No / 0 / 담당자 / 담당부서 / Role Type / Role / 권한 / 경로 
	        { width: 60, id: "RNUM", header: [{ text: "${menu.LN00024}", align:"center"}], align:"center"},
	        { width: 70, id: "checkbox", header: [{ htmlEnable: true, text: "<input type='checkbox' onclick='fnMasterChk(checked)'></input>" }], align: "center", type: "boolean", editable: true, sortable: false, htmlEnable: true},
	        { hidden:true, id: "Photo", header: [{ text: "", align:"center" }], htmlEnable: true, align: "center",
	        	template: function (text, row, col) {
	        		return '<img src="'+row.Photo+'" width="26" height="24">';
	            }
	        },
	        { width: 300, id: "Name", header: [{ text: "${menu.LN00004}", align:"center" }, { content: "inputFilter" }]},
	        {  id: "TeamNM", header: [{ text: "${menu.ZLN0074}", align:"center" }, { content: "inputFilter" }]},
	        { hidden:true, id: "AssignmentTypeName", header: [{ text: "${menu.ZLN0144}", align:"center" }, { content: "selectFilter" }]},
	        { hidden:true, id: "RoleTypeTxt", header: [{text: "${menu.ZLN0104}", align:"center"}, { content:"selectFilter"}]},
	        { hidden:true, id: "AccessRightName", header: [{ text: "${menu.LN00149}", align:"center" }, { content: "selectFilter" }], align:"center"},
	        { hidden:true,id: "Path", header: [{ text: "${menu.LN00043}", align:"center" }, { content: "selectFilter" }], align:"left" },
	        // 등록일 / 우선순위 / Active / Seq
	        { width: 200, id: "AssignedDate", header: [{ text: "${menu.LN00078}", align:"center" }, { content: "selectFilter" }], align:"center"},
	        { width:200, id: "OrderNum", header: [{ text: "${menu.LN00067}", align:"center" }, { content: "selectFilter" }], align:"center", htmlEnable: true},
	        { width: 200, id: "Assignment", header: [{ text: "${menu.ZLN0145}", align:"center" }, { content: "selectFilter"}], align:"center" },
	        {  id: "Seq", header: [{ text: "${menu.ZLN0146}", align:"center" }, { content: "selectFilter" }], hidden:true }
	    ],
	    autoWidth: true,
	    resizable: true,
	    selection: "row",
