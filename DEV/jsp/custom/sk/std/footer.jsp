	...
	$.ajax({
		url: '/sessionTimeOutLogOut.do',
            ...
            error: function(xhr, textStatus, errorThrown) {
            	alert("사용자 세션 시간이 종료되었습니다. 로그인 화면으로 이동합니다.\nUser session time has ended. Go to the login screen.");
    ...
    if (time >= 0) {
        ...
        if (time === 0) {
            sessionTimeOut().then(function(data) {
                clearTimeout(timer);
                alert("사용자 세션 시간이 종료되었습니다. 로그인 화면으로 이동합니다.\nUser session time has ended. Go to the login screen.");
                location.href = "/custom/sk/loginAuth.do"; // 로그아웃
                return;
            }).catch(function(err) {
                console.error(err);
                alert("사용자 세션 시간이 종료되었습니다. 로그인 화면으로 이동합니다.\nUser session time has ended. Go to the login screen.");
        ...
        if(time === 30 && !confirmShown){
        	...
        	dhtmlx.confirm({
        	    title: "Confirmation",
        	    text: "사용자 세션 시간이 30초 남았습니다. 세션 시간을 연장하시겠습니까?\nYou have 30 seconds left for the user session. Do you want to extend the session time?",
