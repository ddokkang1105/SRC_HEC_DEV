...
<div  style="padding: 10px 10px 10px 10px;">
	...
	<div class="cop_hdtitle">
		<ul>
			<li>
				<h3><img src="${root}${HTML_IMG_DIR}/icon_search_title.png">&nbsp;&nbsp;${itemPath}</h3>
	...
	<div id="dictionary_search">
		...
		<colgroup>
		  ...
		  <c:if test="${itemFileOption ne 'VIEWER' and itemFileOption ne 'PDFCNVT' }"> 
			<!-- 첨부 문서 관련 옵션 -->
			<tr>
			<th class="alignL pdL10">${menu.LN00019}</th>
				<td class="alignL pd10 last" style="cursor:pointer" colspan="7">
					<div style="height:70px;width:100%;overflow:auto;overflow-x:hidden;">
						<div class="floatR pdR20">
							<c:if test="${attachFileList.size() > 0}">
							
							<span class="btn_pack medium icon"><span class="download"></span><input value="Download" type="button" onclick="FileDownload()"/></span><br>
		...
		<div class="countList pdT5 pdB5 " >
        	...
        	<li class="floatR">
        		...
        		<c:if test="${sessionScope.loginInfo.sessionAuthLev eq  '1'}">
        		<span class="btn_pack medium icon"><span class="upload"></span><input style="width:89.38px;" value="Upload" type="button" onclick="fnMultiUploadV4()"/></span>
        		<span class="btn_pack medium icon"><span class="del"></span><input style="width:89.38px;" value="Delete" type="button" onclick="FileDelete('checkedFile')"/></span>
        		</c:if>
...
// 		doSearchList();
	...
	function fnReloadGrid(newGridData){
		...
		gridData = result.filter(e => e.ClassCode == "CL11004")
	...
	function fnMultiUploadV4(){
		...
		var data="scrnType=ITM_M&docCategory=ITM&id=${itemID}";
		openPopup(url+"?"+data,480,510,"Attach File");
	}
	
 	function goFileMgt() {
 		
 		  var baseUrl = "goFileMgt.do";
 		  var params = "fileOption=${menuDisplayMap.FileOption}"
	...
	function setSubFrame(){
		...
		var url = "itemTerms.do";
		var target = "standardTermsSchFrm";
		var itemID = "${itemID}";
		var data = "itemID=${itemID}&option=selectHier&unfold=false&strType=1&itemClassMenuUrl=itemTerms&currIdx=undefined&=&arcCode=${arcCode}&showPreNextIcon=Y";
	...
	/* 첨부문서, 관련문서 다운로드 */
	function FileDelete(checkboxName){
