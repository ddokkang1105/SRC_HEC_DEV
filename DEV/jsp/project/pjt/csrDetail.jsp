...
<script type="text/javascript">
	...
	function savePrj() {
		...
		if(confirm("${CM00001}")){
			var url = "admin/saveCSRInfo.do?screenMode=${screenMode}&srID=${srID}&fromSR=${fromSR}&screenType=${screenType}"
	...
	function fnClickedTab(avg, avg2) { 
		...
		if(avg2 == 7){
			...
			var url = avg + ".do";
			var data = "projectID=${ProjectID}&screenMode="+screenMode+"&listEditable="+listEditable+"&screenType=csrDtl&parentID="+parentId+"&authorID=${getMap.AuthorID}";
			var target="tabFrame";
			...
			var screenMode = "${screenMode}"; 
			var url = avg + ".do";
			var data = "projectID=${ProjectID}&DocCategory=PJT&screenType=csrDtl&isPublic=N&screenMode="+screenMode+"&csrEditable="+csrEditable+"&isMember=${isMember}&DocumentID=${ProjectID}&uploadYN=Y&backBtnYN=N";
			var target="tabFrame";
	...
	//	var carUrl = "<%=GlobalVal.CAR_APPROVAL_PATH%>";
		if (avg == "approveInfo" ) {
			...
			// 변경항목 목록으로 이동
			var url = "changeSetInfoListV4.do";
			var data = "csrStatus=${getMap.Status}&screenType=CSR&csrID=${ProjectID}&screenMode=${screenMode}&mainMenu=${mainMenu}&s_itemID=${s_itemID}" 
					...
					+"&url=project/cs/changeSetofCSRV4";
	...
	function fnCompleteCSR(){	
		var url = "admin/updateCSRStatus.do";
	...
	function setSearchMember(authorID){
		var url = "admin/changePjtAuthor.do";
		...
		<tr>
		 ...
		 <th class="alignL pdL10 viewline">${menu.LN00290}</th>
			<td class="alignL pdL10 last" colspan="5" style="height:120px;" valign="Top"> 
				<c:if test="${screenMode == 'E'}"> 
					<div style="width:100%;height:160px;outline:none;">
						<textarea class="tinymceText"  id="Description" name="Description" >${getMap.Description}</textarea>
					</div>
				</c:if>
				<c:if test="${screenMode == 'V'}"> 
				${getMap.Description}
      	...
      	<div class="subtabs">
			<ul>
				...
				<li id="pliugt2"><a href="javascript:fnClickedTab('fileListMgt','2');"><span>${menu.LN00111}</span></a></li> <!--documentGridList-->
