...
<script type="text/javascript">
	...
	function fnSaveNewCSR() {
		...
		if(confirm("${CM00001}")){
			var url = "admin/saveNewCSRInfo.do?srID=${srID}&fromSR=${fromSR}&screenType=${screenType}"
					+"&quickCheckOut=${quickCheckOut}&itemID=${itemID}&speCode=${speCode}&blockSR=${blockSR}&srType=${srType}&checkType=${checkType}";
			ajaxSubmit(document.projectInfoFrm, url, "saveFrame");
		}
	}
	
	// [after Save]
	function goEdit(pjtId, parentID, fromSR, quickCheckOut, itemID, itemAuthorID, checkType) { 
		if(quickCheckOut == "Y"){
			fnAddChangeSet(pjtId, itemID);
		}else if(checkType == "CreateItem"){
			opener.fnCallback(pjtId);
			self.close();
		}else{
	 		...
	 		<tr>
				...
				<td class="alignL pdL10 last" colspan="5" style="height:120px;" valign="Top">
					<div style="width:100%;height:160px;">
						<textarea class="tinymceText" id="Description" name="Description" >${srInfoMap.Comment}</textarea>
					</div>
				</td>
			</tr>
	  		<tr>
