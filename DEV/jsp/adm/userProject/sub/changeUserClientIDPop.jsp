...
<script type="text/javascript">
	...
	function fnEditUserAuth(){	
		if(confirm("${CM00001}")){
			var url  = "admin/editUserClientID.do";
...
<form name="objectInfoFrm" id="objectInfoFrm" action="" method="post" onsubmit="return false;">
	...
	<input type="hidden" id="employeeNums" name="employeeNums" value="${employeeNums}" >
	<div id="objectInfoDiv" class="hidden" style="width:100%;">
		<div class="child_search_head">
			<p><img src="${root}${HTML_IMG_DIR}/category.png"><span>&nbsp;&nbsp;Change Users Site Code</span></p>
		</div>
		<ul id="breadcrumb">
	        <li><span>Select Site Code</span></li>
	    </ul>
		<div id="editArea" style="width:95%;" >
			<table class="tbl_preview mgL10 mgR10" border="0" cellpadding="0" cellspacing="0">
				...
				<tr>
					<th>Site Code</th>
					<td>
						<select id="clientID" name="clientID" style="width:100%;">
							<c:forEach var="clientIDList" items="${clientIDList}" varStatus="status">
								<c:choose>
									<c:when test="${clientIDList.CODE eq 'SYS' and groupManagerYN != null and groupManagerYN ne '' and groupManagerYN eq 'Y'}">
									</c:when>
									<c:otherwise>
										<option value="${clientIDList.CODE}">${clientIDList.NAME}</option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
						</select>
