	...
	if(!selectedCell.length){ 
		...
		if (confirm("${CM00004}")) {//if (confirm("선택된 항목을 삭제하시겠습니까?"))
			...
			for (idx in selectedCell) {
				...
				var i = Number(idx) + 1;
				if (cnt > 0) {
					var id = "ID:" + selectedCell[idx].WFID;
					"<spring:message code='${sessionScope.loginInfo.sessionCurrLangCode}.WM00112' var='WM00112' arguments='"+ id +"'/>"
					alert("${WM00112}");
					chk = 0; 
				} else {
					if (wfIds == "") {
						wfIds = selectedCell[idx].WFID;
					} else{ 
						wfIds = wfIds + "," + selectedCell[idx].WFID;
					}
				}
			}
			
			if (wfIds != "") {
				var url = "admin/delWorkFlow.do";
	...
	var layout = new dhx.Layout("layout", { 
		...
		var grid = new dhx.Grid("grdOTGridArea", {
			columns: [
				...
				{ width: 100, id: "WFID", header: [{ text: "${menu.LN00015}" , align: "center" }], align: "left" },
				{ width: 150, id: "Name", header: [{ text: "${menu.LN00028}", align: "center" }], align: "left" },
				{ hidden: true, width: 100, id: "Category", header: [{ text: "Category", align: "center" }], align: "center" },
				{ width: 150, id: "mgtGRNM", header: [{ text: "Management GR", align: "center" }], align: "left" },
