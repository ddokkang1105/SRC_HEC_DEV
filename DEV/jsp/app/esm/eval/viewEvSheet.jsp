...
<script type="text/javascript">
	...
	var evalSheetID = "${evalSheetID}";
	$(document).ready(function(){
		if(evalSheetID != ""){ //if(screenType == "V"){
		...
		<c:if test="${esType eq 'ITSP' && screenType eq 'E'}">
			getDicData("BTN", "LN0026").then(data => fnSetButton("Submit", "", data.LABEL_NM));
 			// fnSetButton("Submit", "", "등록","primary");
			$(".attr").css("margin","0px");
		</c:if>		
	});
	...
	function fnSaveEv(){
		...
		if(!confirm("${CM00001}")){ return;}
			var url = "saveEv.do?srStatus=${status}&srType="+"${srType}&esType=${esType}";
	...
	function checkMandatory(){
		...
		<c:forEach var="evItemList" items="${evItemList}" varStatus="status">
			...
			attrTypeCode = "${evItemList.AttrTypeCode}";
			mandatory = "${evItemList.Mandatory}";
			id = "${evItemList.EvalItemID}";
			DataType = "${evItemList.DataType}";
			
			if(DataType === "LOV"){
				attrVal = $("input[name='" + attrTypeCode + "']:checked").val();
			} else {
				attrVal = $("[name='" + attrTypeCode + "']").val();
			}
			
			if(mandatory == "1" && (attrVal === "" || attrVal === null || attrVal === undefined)){
				if(msg == ""){
					msg = id;
				} else {
					msg = msg + ", " + id;
		...
		if(msg != ""){
			...
			getDicData("ERRTP", "ZLN0022").then(data => alert(msg + data.LABEL_NM));
		}
		
		return isCheck;
	}
	
	function actionComplete(mode) {
		if(mode == "TAB"){
			fnCallBackSR();
		} else {
			parent.self.close();
...
<form name="evalFrm" id="evalFrm" method="post" action="#" onsubmit="return false;">
	...
	<div id="evalSRDiv" class="border-section">
		<c:choose>
			<c:when test="${esType eq 'ITSP' }">
				<div class="btn-wrap page-subtitle pdB20 pdT20">
					${evTitleName}&nbsp;<c:if test="${screenType eq 'V'}">(Created on : ${creationTime})</c:if>
					<div class="btns">
						<c:if test="${screenType eq 'E'}">
						<button id="Submit" onclick="fnSaveEv()"></button>
						</c:if>
					</div>
				</div>
			</c:when>
		...
		<div class="attr">
			<ul id="evalSRDiv" class="new-form">
				...
				<c:forEach var="evItemList" items="${evItemList}" varStatus="status">
					...
					<li class="attrValue">
						...
						<c:forEach var="LovList" items="${evItemList.LovList}" varStatus="status">
							<input type="radio" name="${evItemList.AttrTypeCode}" id="${evItemList.AttrTypeCode}_${LovList.Score}" value="${LovList.Score}" <c:if test="${screenType ne 'E'}">disabled</c:if> />
							<label for="${evItemList.AttrTypeCode}_${LovList.Score}">&nbsp;${LovList.NAME}&nbsp;&nbsp;</label>
						</c:forEach>
						</c:if>
						<c:if test="${evItemList.DataType eq 'Text' }">
							<textarea style="resize:none; width:100%; height:50px;background: #fff;" name="${evItemList.AttrTypeCode}" <c:if test="${screenType ne 'E'}">readonly</c:if>></textarea>
						</c:if>
					</li>
		...
		<c:if test="${screenType eq 'E' && esType ne 'ITSP'}">
