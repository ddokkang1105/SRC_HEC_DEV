...
// == [Function Definition Section] ====================================================================
	...
	async function getGridHeaderConfig(srTypeArray, srNameArray) {		
		const overviewGridHeaderConfigTemplate = [
			{ width: 120, id: 'ClientName', align: 'center', header: [{ text: 'Company', align: 'center', rowspan: 2 }] },
			
// 			{ width: 80, id: 'ClientShare', align: 'center', header: [{ text: '비중', align: 'center', rowspan: 2 }] },
			
			{ id: 'ClientTotal', header: [{ text: 'Company Overview', align: 'center', colspan: 3 }, { text: 'Total', align: 'center' }], align: 'center', type: 'number' },
			{ id: 'ClientProcessing', header: [{ text: '', align: 'center' }, { text: '${menu.LN00121}', align: 'center' }], align: 'center', type: 'number' },
			{ id: 'ClientCompleted', header: [{ text: '', align: 'center' }, { text: '${menu.LN00118}', align: 'center' }], align: 'center', type: 'number' }
		];
...
//== [Grid Section] ====================================================================
	...
	function loadGrid(REG_STR_DT, REG_END_DT, initTF) {
	    $('#loading').fadeIn(150);
	    searchInProgress = true;

	    async function fetchGridData() {
	        try {
	            // 유효한 SR Type 코드 및 명칭 가져오기
	            ...
	            const response = await $.ajax({
	                url: "getOverviewGridDataByClient.do",
	                type: "POST",
	                data: {
	                    languageID: "${sessionScope.loginInfo.sessionCurrLangType}",
	...
	function handleClickSearchBtn(){
		...
		if(searchInProgress) {
			...
			searchInProgress = true;
		try {
			loadGrid(REG_STR_DT, REG_END_DT);
			}
