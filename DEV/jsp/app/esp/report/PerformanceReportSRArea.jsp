...
<script>
	...
	$(document).ready(function(){
		...
		setupAccumulatedSelect({
		    selectId: "srArea1",
		    nextSelectId: ["srArea2"],
		    containerId: "selectedService",
		    nextContainerID: ["selectedPart"],
		    hiddenId: "service",
		    nextHiddenId: ["part"],
		    callback: [fnGetSRArea2]
		});
	...
	function setCompany(){
		...
		.then(result => {
			...
			fnSelect('srArea1', selectData + "&classCode=CL04001&clientIDs=" + clientIDs, 'getTreeRootItemList', "" , 'Select');
	...
	function fnGetSRArea1(){
		...
		if (customerNo) {
			...
			$("#srArea1").prop("disabled", false);
			fnSelect('srArea1', selectData + "&classCode=CL04001&clientIDs=" + clientIDs, 'getTreeRootItemList', "" , 'Select');
			
		} else {
			$("#srArea1").prop("disabled", true);
			fnSelect('srArea1', selectData + "&classCode=CL04001", 'getTreeRootItemList', "" , 'Select');
		}
		
		var nextSelectId = ["srArea2"];
		var nextContainerID = ["selectedService","selectedPart"];
		var nextHiddenId = ["service","part"];
		var callback = [fnGetSRArea2];
	...
	function doExcelDetail() {
		...
		const fileName2 = "performanceRepotServicePartDetail_" + formattedDateTime;
	...
	function doExcel() {
		...
		const fileName = "performanceRepotServicePart_" + formattedDateTime;
		fnGridExcelDownLoad(excelGrid, "", fileName);
	...
	var grid = new dhx.Grid("grid", {
		 columns: [
			 ...
			 { id: "SRArea1NM", header: [{ text: "${menu.ZLN0188}",align: "center" }], align: "center" },
	        { id: "SRArea2NM", header: [{ text: "${menu.ZLN0079 }", align: "center" }], align: "center" },
	        ...
	        { hidden:true, width: 120, id: "SRArea1", header: [{ text: "srArea1" , align: "center" }], align: "center" },
	        { hidden:true, width: 120, id: "SRArea2", header: [{ text: "srArea2" , align: "center" }], align: "center" },
	...
	grid.events.on("cellClick", function(row,column,e){
		...
		fnSearchDetail(srArea1,srArea2, clientID);
	...
	function fnSearchDetail(srArea1, srArea2, clientID, excel){
		...
		if(excel){
			...
			param += "&srArea1=" + (srArea1 || '');
			param += "&srArea2=" + (srArea2 || '');
		}
	...
	// srArea2 설정
	function fnGetSRArea2(srArea1){
		fnSelect('srArea2', selectData + "&itemID=" + srArea1, 'getChildItemNameList', '', 'Select');
	}
