	...
	<input type="hidden" id="ItemTypeCode" name="ItemTypeCode" value="${ItemTypeCode}" >
 	<div class="title-section flex align-center justify-between bordered-bottom">
   		<span class="flex align-center">
   			...
   			<span id="title">Occurrence list (<span id="TOT_CNT"></span>)</span>
   		</span>
	</div>
...
// 	        			}
	...
	async function fnReload() {
		var sqlID = "model_SQL.getItemMdlOccList";
	    ...
	    const url = "jsonDhtmlxListV7.do?" + params;
...
/*	        if (!result.success) {
	        ...
	        if (result) {
	            fnReloadGrid(dataview, result);
	        } else {
	            fnReloadGrid(dataview, []);
	        }
	    } catch (error) {
