...
public class CustomXssFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(isInvalidMethod(requestMethod)){
    ...
    private boolean isInValidPathAndQuery(HttpServletRequest request) {
        ...
        //uri, query 검사
        if (isInvalidHtmlEntityCheck(requestQuery)) {
            return true;
    ...
    private boolean isInValidQueryWithoutSqlInjection(HttpServletRequest request) {
        ...
        //uri, query 검사
        if (isInvalidSqlInjectionCheck(requestQuery)) result = true;

        return result;
    }

    private boolean isInvalidSqlInjectionCheck(String query) {
    ...
    private boolean isInvalidHtmlEntityCheck(String value) {

        if (value == null) return false;
        return htmlPatterns.matcher(value).find();
    }

    private boolean isInvalidMethod(String requestMethod){

        //1) TRACE/TRACK은 목록 노출 금지: 403/501로만 응답, Allow 헤더 세팅하지 않음
        if ("TRACE".equalsIgnoreCase(requestMethod) || "TRACK".equalsIgnoreCase(requestMethod)
                || !("GET".equalsIgnoreCase(requestMethod) || "POST".equalsIgnoreCase(requestMethod))) return true;
        return false;
    }
